...
BEGIN
  ...
  v_res := F_test_checkcore(errorcode => v_errorcode, errormessage => v_errormessage);
  IF v_errorcode <> 0 THEN
    v_successresponse := '';
    v_finalreport     := '<br><b>Core Test:</b>' || v_errormessage;
  END IF;
  ...
  v_res := F_TEST_checkRTData(errorcode => v_errorcode, errormessage => v_errormessage);
  IF v_errorcode <> 0 THEN
    v_successresponse := '';
    v_finalreport     := v_finalreport || '<br><b>Run Time Data:</b>' || v_errormessage;
  END IF;
