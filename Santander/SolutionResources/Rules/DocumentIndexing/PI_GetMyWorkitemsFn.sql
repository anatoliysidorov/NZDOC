...
BEGIN
  ...
  IF v_workitemId IS NOT NULL THEN
    ...
    v_fromclause   := ' FROM tbl_pi_workitem wi
			INNER JOIN vw_ppl_simpleworkbasketext wbs ON wi.col_id = ' || TO_CHAR(v_workitemId) || ' AND wi.COL_PI_WORKITEMPPL_WORKBASKET = wbs.col_id
  ...
  */
  v_fromclause := ' FROM (SELECT wi.col_id AS Id, wi.col_code AS Code, wi.col_name AS Name, wi.col_title as Title, wi.col_createddate as CreatedDate,
      ...
      f_UTIL_getDrtnFrmNow(wi.col_createddate) AS CreatedDuration, extractValue(c.col_customdata, ''/CONTENT/FROM'') as SourceFrom,
