...
BEGIN
  ...
  IF v_SLAActionId IS NULL THEN
    v_errorCode :=101;
    v_errorMessage :='A SLA Action Id cannot be NULL';
    GOTO cleanup;
  END IF;
  ...
  IF UPPER(v_eventType)='CHANGECASESTATE' AND UPPER(v_subType)='CHANGECASESTATE' THEN
    ...
    IF UPPER(v_subType)='CASETYPES' THEN
        ...
        IF v_paramname1 IS NOT NULL THEN
          FOR recXML IN (SELECT dbms_lob.SUBSTR(substr1, 4000, 1 ) AS substr2 FROM
            ...
            IF  recXML.substr2 IS NOT NULL  OR recXML.substr2<>'' THEN
              SELECT SUBSTR(recXML.substr2, 
                     INSTR(recXML.substr2, '=', 1)+2, 
                     (LENGTH(recXML.substr2)-INSTR(recXML.substr2, '=', 1)-2)
                     ) INTO v_caseTypeData 
              FROM dual;
      ...
      --save parameters
      IF  (v_collectionCaseType IS NOT NULL) AND (v_collectionCaseState IS NOT NULL) AND
          (v_collectionResCode IS NOT NULL) THEN 

        INSERT INTO TBL_AUTORULEPARAMTMPL(COL_PARAMVALUE,COL_PARAMCODE,COL_CODE,
                                          COL_AUTORULEPARAMTPCASETYPE,                                           
                                          COL_AUTORULEPARTPSLAACTIONTP)
        VALUES(v_collectionCaseType, 'COLLECTION_CASE_TYPE_CODE', SYS_GUID(), v_caseTypeId, v_SLAActionId);

        INSERT INTO TBL_AUTORULEPARAMTMPL(COL_PARAMVALUE,COL_PARAMCODE,COL_CODE,
                                          COL_AUTORULEPARAMTPCASETYPE,                                           
                                          COL_AUTORULEPARTPSLAACTIONTP)
        VALUES(v_collectionCaseState, 'COLLECTION_CASE_STATE_CODE', SYS_GUID(), v_caseTypeId, v_SLAActionId);

        INSERT INTO TBL_AUTORULEPARAMTMPL(COL_PARAMVALUE,COL_PARAMCODE,COL_CODE,
                                          COL_AUTORULEPARAMTPCASETYPE,                                           
                                          COL_AUTORULEPARTPSLAACTIONTP)
        VALUES(v_collectionResCode, 'COLLECTION_CASE_RESOLUTION_CODE', SYS_GUID(), v_caseTypeId, v_SLAActionId);
      END IF;     
    END IF;
  END IF; 
  ...
  IF v_paramname1 IS NOT NULL THEN
    for rec in (select column_value as ParamName from table(asf_splitclob(v_paramname1,',')))
    LOOP          
    ...
    INSERT INTO TBL_AUTORULEPARAMTMPL(COL_PARAMVALUE,COL_PARAMCODE,COL_CODE,
                                      COL_AUTORULEPARAMTPCASETYPE,                                       
                                      COL_AUTORULEPARTPSLAACTIONTP)
    VALUES(v_paramvalue,'paramValues', SYS_GUID(), v_caseTypeId, v_SLAActionId);              
  END IF;
  ...
  IF v_paramname1 IS NOT NULL THEN             
    FOR recXML IN (SELECT dbms_lob.SUBSTR(substr1, 4000, 1 ) AS substr2 FROM
        ...
        IF  (recXML.substr2 IS NOT NULL OR recXML.substr2<>'')
        AND (UPPER(recXML.substr2) NOT IN('AS="DATA"', 
                                          'AS="EVENTPARAMS"',
                                          'AS="CASETYPES"',
                                          'AS="TASKTYPEDATA"')
          ...
          IF UPPER(v_eventType)='MAIL' AND UPPER(v_subType)='EMAIL' THEN
            IF UPPER(v_tmpStr)='DISTRIBUTIONCHANNEL' THEN v_tmpStr :='DistributionChannel'; END IF;
            IF UPPER(v_tmpStr)='FROM' THEN v_tmpStr :='From'; END IF;
            IF UPPER(v_tmpStr)='FROM_RULE' THEN v_tmpStr :='FromRule'; END IF;
            IF UPPER(v_tmpStr)='TO' THEN v_tmpStr :='To'; END IF;
            IF UPPER(v_tmpStr)='TO_RULE' THEN v_tmpStr :='ToRule'; END IF;
            IF UPPER(v_tmpStr)='CC' THEN v_tmpStr :='Cc'; END IF;
            IF UPPER(v_tmpStr)='BCC' THEN v_tmpStr :='Bcc'; END IF;
            IF UPPER(v_tmpStr)='TEMPLATE' THEN v_tmpStr :='Template'; END IF;
            IF UPPER(v_tmpStr)='TEMPLATE_RULE' THEN v_tmpStr :='TemplateRule'; END IF;
            IF UPPER(v_tmpStr)='CC_RULE' THEN v_tmpStr :='CcRule'; END IF;
            IF UPPER(v_tmpStr)='ATTACHMENTS_RULE' THEN v_tmpStr :='AttachmentsRule'; END IF;
            IF UPPER(v_tmpStr)='BCC_RULE' THEN v_tmpStr :='BccRule'; END IF;
          END IF;----send email event

          --send sms event          
          IF UPPER(v_eventType)='MESSAGETXT' AND UPPER(v_subType)='INTEGRATION_TWILIO' THEN
            IF UPPER(v_tmpStr)='MESSAGE_CODE' THEN v_tmpStr :='MessageCode'; END IF;
            IF UPPER(v_tmpStr)='MESSAGE_RULE' THEN v_tmpStr :='MessageRule'; END IF;
            IF UPPER(v_tmpStr)='TO' THEN v_tmpStr :='To'; END IF;
            IF UPPER(v_tmpStr)='TO_RULE' THEN v_tmpStr :='ToRule'; END IF;
            IF UPPER(v_tmpStr)='TO_RULE' THEN v_tmpStr :='ToRule'; END IF;
            IF UPPER(v_tmpStr)='FROM' THEN v_tmpStr :='From'; END IF;
          ...
          --save parameters
          IF (v_tmpStr IS NOT NULL) 
          AND UPPER(v_tmpStr) NOT IN ('AS',
                                      'RN',
            ...
            INSERT INTO TBL_AUTORULEPARAMTMPL(COL_PARAMVALUE,COL_PARAMCODE,COL_CODE,
                                              COL_AUTORULEPARAMTPCASETYPE,                                               
                                              COL_AUTORULEPARTPSLAACTIONTP)
            VALUES(v_paramvalue,v_tmpStr, SYS_GUID(), v_caseTypeId, v_SLAActionId);
          END IF;
        END IF;            
     END LOOP;  
  END IF;
