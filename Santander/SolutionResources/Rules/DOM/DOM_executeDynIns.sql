...
begin
  ...
  if v_session is null then
    begin
      select col_dom_cachedom_config, col_session into v_ConfigId, v_session
      from tbl_dom_cache
      where col_dom_cachedom_config = v_ConfigId
      and col_session = v_session
      and col_sorder = (select max(col_sorder) from tbl_dom_cache where col_dom_cachedom_config = v_ConfigId and col_session = v_session and col_sqltype in ('INSERT', 'SEQUENCE'));
      exception
  ...
  loop
    ...
    if v_SqlType = 'INSERT' and instr(v_query,'@' || 'ParentObjectId' || '@') = 0 then
      ...
      ------------------------------------------------------------------
      select min(col_parentrecordid) into v_minrecordid from tbl_dom_cache where col_session = v_session and col_childobject = v_ParentObj and col_sqltype = 'INSERT';
      ...
      --TABLE COL_ID GENERATION SEQUENCE IS NOT USED TO CALCULATE RECORD ID
      /*
      v_recordid := null;
      begin
      ...
      */
      ...
      update tbl_dom_cache set col_parentrecordid = v_recordid where col_session = v_session and col_id in (v_previd, v_id);
    end if;
    v_previd := v_id;
  end loop;
  return 0;
