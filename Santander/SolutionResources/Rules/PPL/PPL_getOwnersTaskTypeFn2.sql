        ...
        FROM   (SELECT 
                ...
                FROM   tbl_dict_tasksystype dtst
                       ...
                       inner join tbl_caseworkerteam cwt
                               ON pbr.col_id = cwt.col_tbl_ppl_team
                       inner join vw_ppl_activecaseworker pcw
                               ON cwt.col_tm_ppl_caseworker = pcw.col_id
                       left  join vw_users au ON pcw.col_userid = au.userid
                       inner join tbl_ppl_workbasket pwb
                               ON pcw.col_id = pwb.col_caseworkerworkbasket
...
and (v_team is null or v_team in (select to_number(column_value) from table(asf_splitclob(s2.TeamIds,','))))
    order by case when lower(:sort) = lower('Id') and lower(:dir) = lower('asc') then s2.Workbasket_Id end asc,
         case when lower(:sort) = lower('Id') and lower(:dir) = lower('desc') then s2.Workbasket_Id end desc,
         case when lower(:sort) = lower('Workbasket_Id') and lower(:dir) = lower('asc') then s2.Workbasket_Id end asc,
         case when lower(:sort) = lower('Workbasket_Id') and lower(:dir) = lower('desc') then s2.Workbasket_Id end desc,
         case when lower(:sort) = lower('Caseworker_Id') and lower(:dir) = lower('asc') then s2.Caseworker_Id end asc,
         case when lower(:sort) = lower('Caseworker_Id') and lower(:dir) = lower('desc') then s2.Caseworker_Id end desc,
         case when lower(:sort) = lower('Caseworker_Name') and lower(:dir) = lower('asc') then s2.CaseWorker_Name end asc,
         case when lower(:sort) = lower('Caseworker_Name') and lower(:dir) = lower('desc') then s2.CaseWorker_Name end desc,
         case when lower(:sort) = lower('Caseworker_Email') and lower(:dir) = lower('asc') then s2.CaseWorker_Email end asc,
         case when lower(:sort) = lower('Caseworker_Email') and lower(:dir) = lower('desc') then s2.CaseWorker_Email end desc,
         case when lower(:sort) = lower('Workbasket_Code') and lower(:dir) = lower('asc') then s2.Workbasket_Code end asc,
         case when lower(:sort) = lower('Workbasket_Code') and lower(:dir) = lower('desc') then s2.Workbasket_Code end desc,
         case when lower(:sort) = lower('Workbasket_Name') and lower(:dir) = lower('asc') then s2.Workbasket_Name end asc,
         case when lower(:sort) = lower('Workbasket_Name') and lower(:dir) = lower('desc') then s2.Workbasket_Name end desc,
         case when lower(:sort) = lower('WorkbasketType_Code') and lower(:dir) = lower('asc') then wbt.col_Code end asc,
         case when lower(:sort) = lower('WorkbasketType_Code') and lower(:dir) = lower('desc') then wbt.col_Code end desc,
         case when lower(:sort) = lower('ParticipantType') and lower(:dir) = lower('asc') then cast(s2.ParticipantType as nvarchar2(255)) end asc,
         case when lower(:sort) = lower('ParticipantType') and lower(:dir) = lower('desc') then cast(s2.ParticipantType as nvarchar2(255)) end desc;
    :cur_item := v_cur;
end;