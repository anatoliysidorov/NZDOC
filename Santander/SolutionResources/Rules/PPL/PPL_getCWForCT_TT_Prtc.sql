...
begin
  ...
  inner join tbl_dict_casesystype dcst on prtc.col_participantcasesystype = dcst.col_id
  inner join tbl_dict_tasksystype dtst on prtc.col_participanttasksystype = dtst.col_id
  inner join tbl_ppl_businessrole pbr on prtc.col_participantbusinessrole = pbr.col_id
  ...
  where dcst.col_id = f_FORM_getParamByName(Input => v_input.getClobVal(), Param => 'CaseTypeId')
    and dtst.col_id = f_FORM_getParamByName(Input => v_input.getClobVal(), Param => 'TaskTypeId')
    and pwb.col_IsPrivate = 0
    and prtc.col_participanttasksystype is null
  union
  ...
  inner join tbl_dict_casesystype dcst on prtc.col_participantcasesystype = dcst.col_id
  inner join tbl_dict_tasksystype dtst on prtc.col_participanttasksystype = dtst.col_id
  inner join tbl_ppl_team ptm on prtc.col_participantteam = ptm.col_id
  ...
  inner join tbl_dict_casesystype dcst on prtc.col_participantcasesystype = dcst.col_id
  inner join tbl_dict_tasksystype dtst on prtc.col_participanttasksystype = dtst.col_id
  inner join tbl_ppl_skill psk on prtc.col_participantppl_skill = psk.col_id
  ...
  inner join tbl_dict_casesystype dcst on prtc.col_participantcasesystype = dcst.col_id
  inner join tbl_dict_tasksystype dtst on prtc.col_participanttasksystype = dtst.col_id
  inner join vw_ppl_activecaseworkersusers pcwu on prtc.col_participantppl_caseworker = pcwu.id
  inner join tbl_ppl_workbasket pwb on pcwu.id = pwb.col_caseworkerworkbasket
  where dcst.col_id = f_FORM_getParamByName(Input => v_input.getClobVal(), Param => 'CaseTypeId')
    and dtst.col_id = f_FORM_getParamByName(Input => v_input.getClobVal(), Param => 'TaskTypeId')
    and pwb.col_IsPrivate = 0
