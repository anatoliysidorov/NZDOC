...
begin
	...
	--calculate personal work basket
	v_WorkbasketIds := TO_CHAR(f_DCM_getMyPersonalWorkbasket());
	IF v_WorkbasketIds IS NULL THEN
		:ErrorCode := 101;
		:ErrorMessage := 'You are missing a personal work basket';
		:TotalCount := 0;
		RETURN;
	END IF;
	
	--get list of all CASE STATEs that are not "closed" if not passing in custom list
	IF v_CaseStateIds IS NULL AND v_MilestoneIds IS NULL THEN
		SELECT listagg(col_id,',') within GROUP(ORDER BY col_id)
		INTO v_CaseStateIds
		FROM TBL_DICT_CASESTATE
		WHERE NVL(COL_ISFINISH, 0) = 0;
	END IF;
