...
begin
  ...
  --DBMS_SESSION.SET_CONTEXT ('CLIENTCONTEXT', 'AccessSubject', '6B67F8C149A5164CE0536C10A8C07818');
  ...
  /*
  if v_WorkbasketIds is null and v_ExternalPartyIds is not null then
      begin
        select col_id into v_calcWB from tbl_ppl_workbasket where COL_WORKBASKETEXTERNALPARTY = v_ExternalPartyIds;
      exception
        when NO_DATA_FOUND then
        v_calcWB := null;
      end;
  ...
  */
  ...
  v_query := 'select ID, rn from (select ID, rownum rn from (select ';
  ...
  if lower(trim(v_sort)) = 'goalsladuration' or lower(trim(v_sort)) = 'goalsladatetime' then
    ...
    v_query := v_query || ' from (select cv.col_id as ID, wb.calcname as Workbasket_Name
                                  FROM tbl_case cv LEFT JOIN vw_ppl_workbasketsimple wb ON cv.col_caseppl_workbasket = wb.id ';
  ...
  ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  ------------------GENERATE SEPARATE SELECT FOR COUNT(*)-----------------------------------------------------------------------------------------------------------------------------------
  ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  v_query2 := ' (select cv.col_id as ID from tbl_case cv ';
  ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  /*
  if v_CaseworkerIds is not null or v_TeamIds is not null or v_ExternalPartyIds is not null then
  ...
  */
  ...
  if v_CaseSysType_Code is not null and v_whereqry is null then
    v_whereqry := ' WHERE cv.col_casedict_casesystype IN (select col_id from tbl_dict_casesystype dcst where dcst.col_code in (select trim(regexp_substr(' || '''' || v_CaseSysType_Code || ''',''[^,]+'',' || '1,level)) c2 from dual connect by level <= length(' || '''' || v_CaseSysType_Code || ''')' || ' - length(replace(' ||  '''' || v_CaseSysType_Code || ''','','',''''))+1))';
  elsif v_CaseSysType_Code is not null and v_whereqry is not null then
    v_whereqry := v_whereqry || ' AND cv.col_casedict_casesystype IN (select col_id from tbl_dict_casesystype dcst where dcst.col_code in (select trim(regexp_substr(' || '''' || v_CaseSysType_Code || ''',''[^,]+'',' || '1,level)) c2 from dual connect by level <= length(' || '''' || v_CaseSysType_Code || ''')' || ' - length(replace(' ||  '''' || v_CaseSysType_Code || ''','','',''''))+1))';
  end if;
  ...
  if v_CaseworkerIds is not null and v_whereqry is null then
    ...
    v_whereqry := ' WHERE cv.col_id in (select cp.col_casepartycase from tbl_caseparty cp where cp.col_casepartycase = cv.col_id and cp.col_casepartyppl_caseworker in (' || v_CaseworkerIds || '))';
    ...
    v_whereqry := v_whereqry || ' AND cv.col_id in (select cp.col_casepartycase from tbl_caseparty cp where cp.col_casepartycase = cv.col_id and cp.col_casepartyppl_caseworker in (' || v_CaseworkerIds || '))';
  end if;
  if v_TeamIds is not null and v_whereqry is null then
    --v_whereqry := ' WHERE cp.col_casepartyppl_team in (' || v_TeamIds || ')';
    v_whereqry := ' WHERE cv.col_id in (select cp.col_casepartycase from tbl_caseparty cp where cp.col_casepartycase = cv.col_id and cp.col_casepartyppl_caseworker in (' || v_TeamIds || '))';
  ...
  if v_ExternalPartyIds is not null and v_whereqry is null then
    ...
    --v_whereqry := v_whereqry || ' AND (cp.col_casepartyexternalparty in (' || v_ExternalPartyIds || ') or cv.col_caseppl_workbasket in (' || v_calcWB || '))';
    v_whereqry := v_whereqry || ' AND cv.col_id in (select cp.col_casepartycase from tbl_caseparty cp where cp.col_casepartycase = cv.col_id and cp.col_casepartyexternalparty in (' || v_ExternalPartyIds || '))';
  end if;
  ...
  v_countquery := 'SELECT COUNT(*) FROM ' || v_query2 || v_whereqry || ')';
  if lower(trim(v_sort)) = 'id' and v_START <= 1000 then
    v_query := v_query || ' where cv.' || v_SORT || ' > 0 order by cv.' || v_SORT || ' ' || v_DIR;
  elsif lower(trim(v_sort)) = 'caseid' and v_START <= 1000 then
    v_query := v_query || ' where cv.' || v_SORT || ' is not null order by cv.' || v_SORT || ' ' || v_DIR;
  elsif lower(trim(v_sort)) = 'summary' and v_START <= 1000 then
    v_query := v_query || ' where cv.' || v_SORT || ' is not null order by cv.' || v_SORT || ' ' || v_DIR;
  elsif lower(trim(v_sort)) = 'casesystype_name' and v_START <= 1000 then
    v_query := v_query || ' where cv.' || v_SORT || ' is not null order by cv.' || v_SORT || ' ' || v_DIR;
  elsif lower(trim(v_sort)) = 'priority_value' and v_START <= 1000 then
    v_query := v_query || ' where cv.' || v_SORT || ' is not null order by cv.' || v_SORT || ' ' || v_DIR;
  elsif lower(trim(v_sort)) = 'ms_statename' and v_START <= 1000 then
    v_query := v_query || ' where cv.' || v_SORT || ' is not null order by cv.' || v_SORT || ' ' || v_DIR;
  elsif lower(trim(v_sort)) = 'workbasket_name' and v_START <= 1000 then
    v_query := v_query || ' order by cv.' || v_SORT || ' ' || v_DIR;
  elsif lower(trim(v_sort)) = 'createddate' and v_START <= 1000 then
    v_query := v_query || ' where cv.' || v_SORT || ' is not null order by cv.' || v_SORT || ' ' || v_DIR;
  elsif lower(trim(v_sort)) = 'createdby_name' then
    v_query := v_query || ' order by cv.' || v_SORT || ' ' || v_DIR;
  else
