...
BEGIN
	...
	FROM (SELECT c1.rn as id1, name1, value1
		 FROM (SELECT rownum as rn, COLUMN_VALUE as name1 FROM TABLE(ASF_SPLITCLOB(:Names_SV, '|||'))) c1
			   LEFT JOIN (SELECT ROWNUM as rn, COLUMN_VALUE as value1 FROM TABLE(ASF_SPLITCLOB(:Values_SV, '|||'))  )  v 
	ON c1.rn=v.rn);
