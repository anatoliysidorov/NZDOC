 ...
 -- v_taskorder := 1;
  ...
  loop
    if v_xmlresult is not null then
      v_xmlresult := v_input.extract(v_path || '/TaskTemplate[' || to_char(v_count) || ']');

      if v_xmlresult is null then
        exit;
      end if;
      v_name := f_UTIL_extract_value_xml(Input => v_xmlresult, Path => 'TaskTemplate[1]/Name/text()');
      v_description := f_UTIL_extract_value_xml(Input => v_xmlresult, Path => 'TaskTemplate[1]/Description/text()');
      v_tasktype    := f_UTIL_extract_value_xml(Input => v_xmlresult, Path => 'TaskTemplate[1]/TaskType/text()');
      v_taskcode    := f_UTIL_extract_value_xml(Input => v_xmlresult, Path => 'TaskTemplate[1]/Code/text()');
      v_taskorder   := f_UTIL_extract_value_xml(Input => v_xmlresult, Path => 'TaskTemplate[1]/TaskOrder/text()');
      v_DeadLine    := f_UTIL_extract_value_xml(Input => v_xmlresult, Path => 'TaskTemplate[1]/DeadLine/text()');
      v_Goal        := f_UTIL_extract_value_xml(Input => v_xmlresult, Path => 'TaskTemplate[1]/Goal/text()');
      v_IconName    := f_UTIL_extract_value_xml(Input => v_xmlresult, Path => 'TaskTemplate[1]/IconName/text()');
      v_ID2         := f_UTIL_extract_value_xml(Input => v_xmlresult, Path => 'TaskTemplate[1]/ID2/text()');
      v_Instatiation:= f_UTIL_extract_value_xml(Input => v_xmlresult, Path => 'TaskTemplate[1]/Instatiation/text()');
      v_MaxAllowed  := f_UTIL_extract_value_xml(Input => v_xmlresult, Path => 'TaskTemplate[1]/MaxAllowed/text()');
      v_Required    := f_UTIL_extract_value_xml(Input => v_xmlresult, Path => 'TaskTemplate[1]/Required/text()');
      v_SystemType  := f_UTIL_extract_value_xml(Input => v_xmlresult, Path => 'TaskTemplate[1]/SystemType/text()');
      v_TaskId      := f_UTIL_extract_value_xml(Input => v_xmlresult, Path => 'TaskTemplate[1]/TaskId/text()');
      v_Type        := f_UTIL_extract_value_xml(Input => v_xmlresult, Path => 'TaskTemplate[1]/Type/text()');
      v_Urgency     := f_UTIL_extract_value_xml(Input => v_xmlresult, Path => 'TaskTemplate[1]/Urgency/text()');
      v_Depth       := f_UTIL_extract_value_xml(Input => v_xmlresult, Path => 'TaskTemplate[1]/Depth/text()');
      v_Leaf        := f_UTIL_extract_value_xml(Input => v_xmlresult, Path => 'TaskTemplate[1]/Leaf/text()');
      v_ProcCode    := f_UTIL_extract_value_xml(Input => v_xmlresult, Path => 'TaskTemplate[1]/ProcCode/text()');
      v_PageCode    := f_UTIL_extract_value_xml(Input => v_xmlresult, Path => 'TaskTemplate[1]/PageCode/text()');
      v_isHidden    := f_UTIL_extract_value_xml(Input => v_xmlresult, Path => 'TaskTemplate[1]/IsHidden/text()');
      BEGIN     
      SELECT col_id 
             INTO 
             v_executionmethodtasktemplate
      FROM tbl_dict_executionmethod 
      WHERE col_code = f_UTIL_extract_value_xml(Input => v_xmlresult, Path => 'TaskTemplate[1]/ExecutionMethod/text()');
      EXCEPTION 
      ...
      END;    
      v_icon := f_UTIL_extract_value_xml(Input => v_xmlresult, Path => 'TaskTemplate[1]/Icon/text()');
      v_iconcls := f_UTIL_extract_value_xml(Input => v_xmlresult, Path => 'TaskTemplate[1]/IconCls/text()');
      ...
      begin 
          ...
          WHERE col_ucode = f_UTIL_extract_value_xml(Input => v_xmlresult, Path => 'TaskTemplate[1]/TaskState/text()');
      EXCEPTION 
          WHEN NO_DATA_FOUND THEN 
            v_taskState := NULL;
   ...
   p_util_update_log ( XmlIdLog => XmlId, Message => 'TaskTemplate       Merged TBL_TASKTEMPLATE with name'||v_name, IsError => 0);  
      ...
      /*************************************************************/
       SELECT y.column_value.getstringval() 
          INTO v_cnt
       FROM xmltable('count(/TaskTemplate[1]/TaskStateInitiation)' passing v_xmlresult) y ;
       FOR i IN 1..v_cnt LOOP 
            v_assignprocessorcode := f_UTIL_extract_value_xml(Input => v_xmlresult, Path => 'TaskTemplate[1]/TaskStateInitiation['||i||']/AssignProcessorCode/text()');
            v_processorcode := f_UTIL_extract_value_xml(Input => v_xmlresult, Path => 'TaskTemplate[1]/TaskStateInitiation['||i||']/ProcessorCode/text()');
            v_MapTskstinitInitmtd := f_UTIL_extract_value_xml(Input => v_xmlresult, Path => 'TaskTemplate[1]/TaskStateInitiation['||i||']/MapTskstinitInitmtd/text()');
            v_MapTskstinitTsks := f_UTIL_extract_value_xml(Input => v_xmlresult, Path => 'TaskTemplate[1]/TaskStateInitiation['||i||']/MapTskstinitTsks/text()');
            ...
            v_code := f_UTIL_extract_value_xml(Input => v_xmlresult, Path => 'TaskTemplate[1]/TaskStateInitiation['||i||']/Code/text()');
            MERGE INTO tbl_map_taskstateinitiation
            ...
            WHEN MATCHED THEN
              UPDATE SET col_assignprocessorcode = assignprocessorcode, col_processorcode = processorcode,
                         col_map_taskstateinittasktmpl = nextparentid, col_map_tskstinit_initmtd = MapTskstinitInitmtdID,
                         col_map_tskstinit_tskst = MapTskstinitTsksID
            WHEN NOT MATCHED THEN
            INSERT
            (col_assignprocessorcode, col_processorcode , col_map_taskstateinittasktmpl, col_map_tskstinit_initmtd,
             col_map_tskstinit_tskst, col_code)
            VALUES
   ...
   p_util_update_log ( XmlIdLog => XmlId, Message => 'TaskTemplate       Merged TBL_MAP_TASKSTATEINITIATION with code'||v_code, IsError => 0);                
            ...
            SELECT col_id INTO v_col_id FROM tbl_map_taskstateinitiation WHERE col_code = v_code;
         ...
         v_path_2 := 'count(/TaskTemplate[1]/TaskStateInitiation['||i||']/TaskEvent)';  
         SELECT y.column_value.getstringval() 
          INTO v_cnt_2
       FROM xmltable(v_path_2 passing v_xmlresult) y;
                    
       FOR j IN 1..v_cnt_2  LOOP
              v_TEProcessorCode := f_UTIL_extract_value_xml(Input => v_xmlresult, Path => 'TaskTemplate[1]/TaskStateInitiation['||i||']/TaskEvent['||j||']/ProcessorCode/text()');
              v_TaskEventOrder  := f_UTIL_extract_value_xml(Input => v_xmlresult, Path => 'TaskTemplate[1]/TaskStateInitiation['||i||']/TaskEvent['||j||']/TaskEventOrder/text()');
              v_TaskEventMoment := f_UTIL_extract_value_xml(Input => v_xmlresult, Path => 'TaskTemplate[1]/TaskStateInitiation['||i||']/TaskEvent['||j||']/TaskEventMoment/text()');
              v_TaskEventType   := f_UTIL_extract_value_xml(Input => v_xmlresult, Path => 'TaskTemplate[1]/TaskStateInitiation['||i||']/TaskEvent['||j||']/TaskEventType/text()');
              v_code_event      := f_UTIL_extract_value_xml(Input => v_xmlresult, Path => 'TaskTemplate[1]/TaskStateInitiation['||i||']/TaskEvent['||j||']/Code/text()'); 
                   ...
                   WHERE col_code =  v_TaskEventType;  
                     ...
                     WHEN MATCHED THEN 
                       UPDATE  SET col_processorcode =TEProcessorCode , col_taskeventorder =TaskEventOrder, 
                                   col_taskeventmomenttaskevent =TaskEventMomentID , col_taskeventtaskstateinit =cl_id , 
                                   col_taskeventtypetaskevent = TaskEventTypeID  
                     WHEN NOT MATCHED THEN 
                       INSERT (col_processorcode , col_taskeventorder, col_taskeventmomenttaskevent , col_taskeventtaskstateinit , col_taskeventtypetaskevent, col_code ) 
   ...
   p_util_update_log ( XmlIdLog => XmlId, Message => 'TaskTemplate       Merged TBL_TASKEVENT with code'||v_code_event, IsError => 0);                         
                   ...
                   /* INSERT INTO tbl_taskevent
                    (col_processorcode , col_taskeventorder, col_taskeventmomenttaskevent , col_taskeventtaskstateinit , col_taskeventtypetaskevent, col_code  )
                    VALUES
                    (v_TEProcessorCode,v_TaskEventOrder , v_TaskEventMomentID, v_col_id,  v_TaskEventTypeID, v_code_event);
                      */
...
--      v_taskorder := v_taskorder + 1;
      begin
        v_result := f_UTIL_extract_values_recurs(Input => v_input/*.getClobVal()*/, 
        	                                     Path => v_path || '/TaskTemplate[' || to_char(v_count) || ']', 
