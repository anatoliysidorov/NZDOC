...
BEGIN
    ...
    BEGIN 
        ...
        INNER JOIN tbl_dict_casesystype cst ON ap.casesystype = cst.col_id
        INNER JOIN tbl_case cs ON cst.col_id = cs.col_casedict_casesystype
        WHERE cs.col_id = v_id
                AND lower(ap.PAGETYPE_CODE) = lower('FULL_PAGE_CASE_DETAIL');
    EXCEPTION
        WHEN NO_DATA_FOUND THEN
          NULL;
    END;
    ...
    IF v_TARGET_RAWTYPE IS NOT NULL AND v_TARGET_ELEMENTID IS NOT NULL THEN
        IF upper(v_TARGET_RAWTYPE) = 'PAGE' THEN
          ...
          v_pageParams := '<PageParams>' ||
                            '<Case_Id>' || v_id || '</Case_Id>' ||
                            '<app>CaseDetailRuntime</app>' ||
                            '<group>FOM</group>' ||
        ...
        END IF;
    END IF;  
    v_customData := f_DCM_getCaseCustomData(v_id);
