...
BEGIN
  BEGIN
    SELECT allowed
      INTO v_result
      FROM TABLE(F_dcm_getcaseworkeraccessfn2(p_accessobjectid => :AccessObjectId,
                                              p_caseid         => NULL,
                                              ...
                                              p_permissionid   => (SELECT col_id
                                                                    ...
                                                                    WHERE col_code = :AccessType
                                                                      AND col_permissionaccessobjtype =
                                                                          (SELECT col_id FROM tbl_ac_accessobjecttype WHERE col_code = :AccessObjectType)),
