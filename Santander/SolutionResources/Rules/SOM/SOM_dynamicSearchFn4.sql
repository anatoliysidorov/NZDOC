  ...
  LOOP
    ...
    IF (rec.LevelPath = 1 AND rec.ParentObjectId <> v_rootobjid AND rec.ChildObjectId <> v_rootobjid) THEN
      ...
      LOOP
        ...
        -------------------------------------------------
          IF v_PathAdded(recJoin.PathId) = 0 and nvl(v_ObjectAdded(recJoin.ParentObjectId),0) = 0 and nvl(v_ObjectAdded(recJoin.ChildObjectId),0) = 1 THEN
  ...
  LOOP
    IF rec.columnComp = 1 THEN
      BEGIN
        ...
        v_expleft := NULL;
      END;
      v_expleft := /*v_ObjectAlias(v_return)*/ rec.leftAlias || '.' || v_expleft;
      BEGIN
        ...
        v_expright := NULL;
      END;
      v_expright := /*v_ObjectAlias(v_return)*/ rec.rightAlias || '.' || v_expright;
