DECLARE
  ...
  v_expr               NVARCHAR2(255);
  v_exprstart          NVARCHAR2(255);
  v_exprend            NVARCHAR2(255);
  v_expleft            NVARCHAR2(255);
  v_expright           NVARCHAR2(255);
  v_ObjectAdded        p_ObjectAdded.ObjAddedType;
  v_SearchAttrAdded    p_ObjectAdded.ObjAddedType;
  v_PathAdded          p_ObjectAdded.ObjAddedType;
  --v_ObjectAdded        NUMBER_ARRAY := NUMBER_ARRAY(32767);
  --v_SearchAttrAdded    NUMBER_ARRAY := NUMBER_ARRAY(32767);
  ---
...
BEGIN
  ...
  v_return := p_ObjectAdded.f_SOM_setObjectAdded(ObjectAdded => v_ObjectAdded, objectid => v_rootobjid);
  ...
  LOOP
    ...
    IF (rec.LevelPath = 1 AND rec.ParentObjectId <> v_rootobjid AND rec.ChildObjectId <> v_rootobjid) THEN
      ...
      LOOP
        ----------------------------------------------------------------------------------------------------------------------------------------------------------
        v_return := p_ObjectAdded.f_SOM_getobjectadded(ObjectAdded => v_ObjectAdded, objectid => recJoin.ChildObjectId);
        v_return2 := p_ObjectAdded.f_SOM_getobjectadded(ObjectAdded => v_ObjectAdded, objectid => recJoin.ParentObjectId);
        ----------------------------------------------------------------------------------------------------------------------------------------------------------
        IF (v_rootobjid = recJoin.ParentObjectId) THEN
          IF v_PathAdded(recJoin.PathId) = 0 THEN
          /*IF(p_ObjectAdded.f_SOM_getobjectadded(ObjectAdded => v_ObjectAdded, objectid => recJoin.ChildObjectId) = 0)
            and (p_ObjectAdded.f_SOM_getobjectadded(ObjectAdded => v_ObjectAdded, objectid => recJoin.ParentObjectId) = 1)*/
            ...
            v_return := p_ObjectAdded.f_SOM_setobjectadded(ObjectAdded => v_ObjectAdded, objectid => recJoin.ChildObjectId);
        ...
        -------------------------------------------------
          IF v_PathAdded(recJoin.PathId) = 0 THEN
          /*IF(p_ObjectAdded.f_SOM_getobjectadded(ObjectAdded => v_ObjectAdded, objectid => recJoin.ParentObjectId) = 0)
            and (p_ObjectAdded.f_SOM_getobjectadded(ObjectAdded => v_ObjectAdded, objectid => recJoin.ChildObjectId) = 1)*/
            v_fromqry := v_fromqry || f_dcm_getPathJoinType(PathJoinType => recJoin.JoinType) || ' join ' || recJoin.ParentTableName || ' ' || recJoin.ParentAlias ||
            ' on ' || recJoin.ChildAlias || '.' || recJoin.ForeignKeyName || ' = ' || recJoin.ParentAlias || '.col_id';
            v_return := p_ObjectAdded.f_SOM_setObjectAdded(ObjectAdded => v_ObjectAdded, objectid => recJoin.ParentObjectId);
            v_PathAdded(recJoin.PathId) := 1;
    ...
    ----------------------------------------------------------------------------------------------------------------------------------------------------------
    v_return := p_ObjectAdded.f_SOM_getobjectadded(ObjectAdded => v_ObjectAdded, objectid => rec.ChildObjectId);
    v_return2 := p_ObjectAdded.f_SOM_getobjectadded(ObjectAdded => v_ObjectAdded, objectid => rec.ParentObjectId);
    ----------------------------------------------------------------------------------------------------------------------------------------------------------
    IF (v_rootobjid = rec.ParentObjectId AND rec.ResAttr = 1) THEN
      IF v_PathAdded(rec.PathId) = 0 THEN
      /*IF(p_ObjectAdded.f_SOM_getobjectadded(ObjectAdded => v_ObjectAdded, objectid => rec.ChildObjectId) = 0)
             and (p_ObjectAdded.f_SOM_getobjectadded(ObjectAdded => v_ObjectAdded, objectid => rec.ParentObjectId) = 1)*/
        ...
        v_return := p_ObjectAdded.f_SOM_setObjectAdded(ObjectAdded => v_ObjectAdded, objectid => rec.ChildObjectId);
    ...
    IF (v_rootobjid = rec.ChildObjectId AND NVL(v_relid, 0) <> rec.RelId) THEN
      /*IF(p_ObjectAdded.f_SOM_getobjectadded(ObjectAdded => v_ObjectAdded, objectid => rec.ParentObjectId) = 0)
             and (p_ObjectAdded.f_SOM_getobjectadded(ObjectAdded => v_ObjectAdded, objectid => rec.ChildObjectId) = 1)*/
      IF v_PathAdded(rec.PathId) = 0 THEN
        v_fromqry := v_fromqry || f_dcm_getPathJoinType(PathJoinType => rec.JoinType) || ' join ' || rec.ParentTableName || ' ' || rec.ParentAlias ||
        ' on ' || rec.ChildAlias || '.' || rec.ForeignKeyName || ' = ' || rec.ParentAlias || '.col_id';
        v_return := p_ObjectAdded.f_SOM_setObjectAdded(ObjectAdded => v_ObjectAdded, objectid => rec.ParentObjectId);
        v_PathAdded(rec.PathId) := 1;
      ...
      LOOP
        ...
        IF (F_FORM_GETPARAMBYNAME(v_input, rec2.AttrCode) IS NOT NULL) THEN
          ...
          ELSE
            ...
            v_exprend := '';
          END IF;
          IF (v_data_type_where = 'date') THEN
          ...
          IF (NVL(rec2.SearchAttrLike, 0) = 0) THEN
            ...
            ELSIF (v_data_type_where = 'date') THEN
              v_expr := ' = ' || v_exprstart || '''' || F_FORM_GETPARAMBYNAME(v_input, rec2.AttrCode) || '''' || v_exprend;
            ELSE
              v_expr := ' = ' || v_exprstart || '''' || F_FORM_GETPARAMBYNAME(v_input, rec2.AttrCode) || '''' || v_exprend;
            END IF;
          ...
          IF (v_rootobjid = recJoin.ParentObjectId) THEN
            ...
            /*IF(p_ObjectAdded.f_SOM_getobjectadded(ObjectAdded => v_ObjectAdded, objectid => recJoin.ChildObjectId) = 0)
             and (p_ObjectAdded.f_SOM_getobjectadded(ObjectAdded => v_ObjectAdded, objectid => recJoin.ParentObjectId) = 1)*/
              v_fromqry := v_fromqry || f_dcm_getPathJoinType(PathJoinType => recJoin.JoinType) || ' join ' || recJoin.ChildTableName || ' ' || recJoin.ChildAlias ||
              ' on ' || recJoin.ChildAlias || '.' || recJoin.ForeignKeyName || ' = ' || recJoin.ParentAlias || '.col_id';
              v_count := p_ObjectAdded.f_SOM_setObjectAdded(ObjectAdded => v_ObjectAdded, objectid => recJoin.ChildObjectId);
              v_PathAdded(recJoin.PathId) := 1;
              v_ObjectsAdded := 1;
            END IF;
          ELSIF v_parentobjectid = recJoin.ParentObjectId THEN
  ...
  FOR rec IN (SELECT resattr.COL_ID AS ResultAttrId,
                     ...
                     and col_childfom_relfom_object in (select col_id from tbl_fom_object where col_id = v_rootobjid and col_id in
  ...
  LOOP
    IF (v_xmlalias IS NULL OR v_xmlalias <> rec.ObjXMLAlias) THEN
      v_xmlfromqry := ' ,xmltable(''/CustomData/Attributes'' passing ' || /*rec.ObjAlias*/ F_FORM_GETALIASFROMQUERY(v_fromqry, rec.ObjTableName) ||
  ...
  LOOP
    IF rec.columnComp = 1 THEN
      BEGIN
        SELECT fo.COL_ALIAS || '.' || fa.COL_COLUMNNAME as ColumnReference INTO v_expleft
      ...
      END;
      BEGIN
        SELECT fo.COL_ALIAS || '.' || fa.COL_COLUMNNAME as ColumnReference INTO v_expright
        FROM TBL_FOM_ATTRIBUTE fa
        INNER JOIN TBL_FOM_OBJECT fo on fa.COL_FOM_ATTRIBUTEFOM_OBJECT = fo.COL_ID
    ...
    IF (F_FORM_GETPARAMBYNAME(v_input, v_curentSearchAttr) IS NOT NULL OR (v_data_type_where = 'date' AND (v_dateFrom IS NOT NULL OR v_dateTo IS NOT NULL))) THEN
      v_curentParam := F_FORM_GETPARAMBYNAME(v_input, v_curentSearchAttr);
      IF (UPPER(rec.UIElementType) = 'MULTISELECT_COMBOBOX') THEN
        ...
        v_expInEnd := ''','','')))';
      ELSIF (NVL(rec.Constant, 0) <> 0 AND v_data_type_where = 'text' AND UPPER(rec.UIElementType) = 'TEXT_FIELD' AND v_curentParam = :AccessSubjectCode) THEN
        v_curentParam := :AccessSubjectCode;
        v_condition := ' = ';
        v_expInSelect := '';
        v_expInColValue := '';
        v_expInFrom := '';
        v_expInEnd := '';
      ELSIF (NVL(rec.Constant, 0) <> 0 AND v_data_type_where = 'integer' AND (NVL(INSTR(v_curentParam, ';'), 0) <> 0 OR NVL(INSTR(v_curentParam, ','), 0) <> 0)) THEN
        v_condition := ' in ';
        v_expInSelect := '';
        v_expInColValue := '';
        v_expInFrom := '(';
        v_expInEnd := ')';
        v_curentParam := REPLACE(v_curentParam, ';', ',');
      ELSE
        v_condition := ' = ';
        v_expInSelect := '';
        v_expInColValue := '';
      ...
      IF NVL(rec.SearchAttrLike, 0) = 0 THEN
        IF (v_data_type_where = 'integer') THEN
          IF (TRIM(v_condition) = 'in') THEN
            v_expr := v_condition || v_expInSelect || v_exprstart || v_expInColValue || v_exprend || v_expInFrom || v_curentParam || v_expInEnd;
          ELSE
            v_expr := v_condition || v_exprstart || v_curentParam || v_exprend;
          END IF;
        ELSIF (v_data_type_where = 'date') THEN
          IF (v_dateFrom IS NOT NULL AND v_dateTo IS NOT NULL) THEN
            ...
            WHILE (v_count < 2)
              LOOP
                IF (v_count = 0) THEN
                  v_curentParam := v_dateFrom;
                  v_condition := '>=';
                ELSE
                  v_curentParam := v_dateTo;
                  v_condition := '<=';
                END IF;
                ...
                IF (REGEXP_SUBSTR(v_dateFrom, '(\d{1,2})(-)([a-zA-Z]{3})(-)(\d{4})$') IS NOT NULL) THEN
                  ...
                  ELSE --is simple storage
                    v_expr := v_condition || 'to_date(' || '''' || v_curentParam || '''' || ', ''' || v_date_format || ''')';
                  END IF;
                ELSE -- system format 
                  IF (rec.AttrStorageType = 'XML') THEN
                    v_expr := v_condition || 'trunc(to_date(' || '''' || v_curentParam || '''' || ', ''' || v_date_format_system || '''))';
                  ELSE --is simple storage
                    v_expr := v_condition || v_exprstart || 'to_date(' || '''' || v_curentParam || '''' || ', ''' || v_date_format_system || ''')' || v_exprend;
                  END IF;
                END IF;
          ...
          ELSE
            ...
            IF (REGEXP_SUBSTR(v_curentParam, '(\d{1,2})(-)([a-zA-Z]{3})(-)(\d{4})$') IS NOT NULL) THEN
              IF (rec.AttrStorageType = 'XML') THEN
                v_expr := v_condition || v_exprstart || '''' || v_curentParam || '''' || v_exprend;
              ELSE --is simple storage
                v_expr := v_condition || 'to_date(' || '''' || v_curentParam || '''' || ', ''' || v_date_format || ''')';
      ...
      ELSIF (rec.SearchAttrLike = 1) THEN
        v_expr := ' like ' || v_exprstart || '''%' || v_curentParam || '%''' || v_exprend;
      ELSE
        v_expr := NULL;
      END IF;
    END IF;
  ...
  if v_limit_row is not null and v_whereqry is null then
    v_whereqry := ' where rownum <= ' || (v_star_row + v_limit_row);
  elsif v_limit_row is not null and v_whereqry is not null then
    v_whereqry := v_whereqry || ' and rownum <= ' || (v_star_row + v_limit_row);
  end if;
  --------------------------------------------------------------------------------
  v_query := v_srchqry || v_fromqry || v_xmlfromqry || v_whereqry || v_orderBy;

  IF (v_limit_row IS NOT NULL) THEN
    v_query := 'select rownum as rn, tbl.* from (' || v_query || ' ) tbl ';
    v_query := 'select * from (' || v_query || ') where rn > ' || v_star_row || ' and rn <= ' || (v_star_row + v_limit_row);
  END IF;
