...
begin
  ...
  v_result := f_EMAIL_PI_getMyWorkitemsFn(createdEnd => v_createdEnd, createdStart => v_createdStart, DIR => v_dir, ErrorCode => v_ErrorCode, ErrorMessage => v_ErrorMessage,
                                    FIRST => v_START, Id => v_id, ITEMS => v_ITEMS, LIMIT => v_LIMIT, NAME => v_name, searchCode => v_searchCode, SORT => v_sort,
                                    sourceType => v_sourceType, StateCode => v_stateCode, Title => v_title, TotalCount => v_TotalCount, workbasketId => v_workbasketId, WorkitemId => v_workitemId, EmailFrom => v_from, Customer => v_customer, Owner => v_owner);
