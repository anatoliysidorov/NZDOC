...
where cw.col_id in (select CaseworkerId from table(f_DCM_getProxyAssignorList()))
union all
...
where (s7.PeopleType in ('CASEWORKER', 'BUSINESSROLE', 'TEAM', 'SKILL')
  and s7.CaseworkerId in (select CaseworkerId from table(f_DCM_getProxyAssignorList()))) or
