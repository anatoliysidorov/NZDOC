...
begin
  ...
  v_TaskComplete := 1;
  begin
    select tst.col_id, cwu.Id, to_char(trunc(dee.col_datevalue, 'D')), (dee.col_datevalue - des.col_datevalue) * 24 * 60
  ...
  if (v_TaskComplete is not null) and (v_TaskSysType is not null) and (v_CaseWorkerId is not null) and (v_YearMonth is not null) then
    begin
      select col_id into v_id
        from tbl_statweek
        where col_statweekdict_tasksystype = v_TaskSysType
        and col_statweekppl_caseworker = v_Caseworkerid
    ...
    if v_id is not null then
      begin
        select col_avginprocess, col_totalinprocess, col_totalclosed into v_AvgInProcess, v_TotalInProcess, v_TotalClosed from tbl_statweek where col_id = v_id;
        exception
          when NO_DATA_FOUND then
            v_AvgInProcess := 0;
            v_TotalInProcess := 0;
      ...
      insert into tbl_statweek(col_statweekppl_caseworker, col_statweekdict_tasksystype, col_yearmonth, col_totalclosed, col_meetgoal, col_nomeetgoal, col_meetdeadline, col_nomeetdeadline, col_avginprocess, col_totalinprocess)
