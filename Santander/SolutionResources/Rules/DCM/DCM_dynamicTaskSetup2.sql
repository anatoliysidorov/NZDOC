...
begin
  ...
  --SETUP DEPENDENCY BETWEEN TASKS
  begin
    select tsi.col_id into v_ChildTSIId from tbl_map_taskstateinitiation tsi
      ...
      where tsi.col_taskstateinitdynamictask in (select col_id from tbl_dynamictask
        ...
        and col_dynamictaskparentid <> 0 and col_dynamictaskparentid is not null
        and col_id = (select max(col_id) from tbl_dynamictask where col_sessionid = v_SessionId))
      and ts.col_activity = v_StartedActivityCode;
    exception
      when NO_DATA_FOUND then
        v_ChildTSIId := null;
        return -1;
  end;
  for rec in (select tsi.col_id as ParentTSIId
                  ...
                  and col_id < (select max(col_id) from tbl_dynamictask where col_sessionid = v_SessionId))
               and ts.col_activity = v_ClosedActivityCode)
  loop
    insert into tbl_taskdependency(col_tskdpndchldtskstateinit, col_tskdpndprnttskstateinit, col_type, col_code) values(v_ChildTSIId, rec.ParentTSIId, v_TaskDependencyType, sys_guid());
  end loop;
  --SETUP TASK EVENTS
  v_ProcessorCode := 'f_DCM_actionResolveWF';
