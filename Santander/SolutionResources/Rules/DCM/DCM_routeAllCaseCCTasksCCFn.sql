...
begin
    ...
    for rec in(select  s1.TaskId as TaskId,
             ...
             s1.twId as twId,
             s1.twActivity as twActivity,
             s1.twTargetActivity as twTargetActivity,
             s1.TaskStateInitId as TaskStateInitId,
             s1.TaskStateInitProc as TaskStateInitProc,
             s1.TaskStateInitAssignProc as TaskStateInitAssignProc
    from    (select    chldtsk.col_id as TaskId,
                        chldtsk.col_taskid as TaskBsId,
                        chldtsk.col_name as TaskName,
                        chlddtst.col_code as TaskSysType,
                        chldtsk.col_type as TaskType,
                        chldtsk.col_dateassigned as TaskDateAssigned,
                        chldtsk.col_dateclosed as TaskDateClosed,
                        chldtsk.col_depth as TaskDepth,
                        chldtsk.col_description as TaskDescription,
                        chldtwi.col_id as twId,
                        chldtwi.col_activity as twActivity,
                        chlddts.col_activity as twTargetActivity,
                        chldmtsi.col_id as TaskStateInitId,
                        chldmtsi.col_processorcode as TaskStateInitProc,
                        chldmtsi.col_assignprocessorcode as TaskStateInitAssignProc,
                        chldtsk.col_id as ChildTaskId,
             ...
             inner join tbl_dict_tasktransition dtt      on dtt.col_sourcetasktranstaskstate = chldtwi.col_tw_workitemccdict_taskst and dtt.col_targettasktranstaskstate = chlddts.col_id and nvl(dtt.col_manualonly,0) = 1
             ...
             where      tsk.col_id = v_TaskId
                        ...
                        and chlddts.col_activity in(select NextActivity
                        from    table(f_DCM_getNextActivityListCC(TaskId => chldtsk.col_id)))
    ...
    loop
        ...
        if v_IsValid = 1 then
            begin
                ...
                where  col_id = rec.TaskStateInitId;
                ...
                loop
                    if rec2.taskstatesetupcode = 'DATESTARTED' then
                        ...
                        if rec2.ForcedNull = 1 then
                            update tbl_taskcc
                            set    col_taskccppl_workbasket = null
                            where  col_id = rec.TaskId;
                ...
                if rec.twTargetActivity = v_stateAssigned then
                    v_result := f_DCM_invokeTaskAssignProc(ProcessorName => rec.TaskStateInitAssignProc,
                                                           TaskId => rec.TaskId);
                end if;
                v_result := f_DCM_addTaskDateEventCCList(TaskId => rec.TaskId,
                                                         state => rec.twTargetActivity);
            exception
            when NO_DATA_FOUND then
                exit;
            end;
