...
begin
  ...
  loop
    ...
    if v_processedstatus < 8 then
      begin
        select col_casetask into v_CaseId from tbl_task where col_id = rec.col_taskeventqueuetask;
        exception
        when NO_DATA_FOUND then
          v_CaseId := null;
      end;
    ...
    if v_errorstatus = 1 then
      update tbl_tw_workitem set col_activity = v_stateResolved, col_tw_workitemprevtaskstate = col_tw_workitemdict_taskstate, col_tw_workitemdict_taskstate = (select col_id from tbl_dict_taskstate where col_activity = v_stateResolved)
        where col_id = (select col_tw_workitemtask from tbl_task where col_id = rec.col_taskeventqueuetask);
      --SET TASK DATE EVENT
