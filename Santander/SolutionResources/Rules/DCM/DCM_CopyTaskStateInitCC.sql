...
begin
  ...
  begin
   insert into tbl_map_taskstateinitcc(col_map_taskstateinitcctaskcc,col_processorcode,col_assignprocessorcode,col_map_tskstinitcc_initmtd,col_map_tskstinitcc_tskst,
   ...
   exception
     when DUP_VAL_ON_INDEX then
       :ErrorCode := 100;
       :ErrorMessage := 'DCM_CopyTaskStateInitCC: ' || SUBSTR(SQLERRM, 1, 200);
       return -1;
     when OTHERS then
       :ErrorCode := 100;
       :ErrorMessage := 'DCM_CopyTaskStateInitCC: ' || SUBSTR(SQLERRM, 1, 200);
