with spos as
(select col_id as PermissionId, col_defaultACL as DefaultACL, col_orderACL as OrderACL, col_position-1 as position from tbl_ac_permission where col_id = :p_PermissionId)
...
nvl(s9.PermissionId, :p_PermissionId) as PermissionId, nvl(s9.AccessObjectId, :p_AccessObjectId) as AccessObjectId,
...
from
(select col_id as PermissionId, col_defaultACL as DefaultACL, col_orderACL as OrderACL from tbl_ac_permission where col_id = :p_PermissionId) s10
inner join
...
where cw.col_id = :p_CaseworkerId
union all
...
acl.col_id as ACLId,
(case when aot.col_code = 'CASE_BUSINESS_OBJECT' and acl.col_processorcode is not null then f_dcm_invokeaclcaseproc(CaseId => :p_CaseId, ProcessorName => acl.col_processorcode)
      when aot.col_code = 'TASK_BUSINESS_OBJECT' and acl.col_processorcode is not null then f_dcm_invokeacltaskproc(ProcessorName => acl.col_processorcode, TaskId => :p_TaskId) else acl.col_aclentry end) as ACLEntry
...
where s8.CaseworkerId = :p_CaseworkerId and s8.AccessObjectId = :p_AccessObjectId
