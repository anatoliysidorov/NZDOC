...
begin
  ...
  v_result := f_DBG_createDBGTrace(CaseId => v_CaseId, Location => 'DCM_processEventCC4 before validation begins',
                                   Message => 'Task ' || to_char(v_NextTaskId) || ' before validation event processing', Rule => 'DCM_processEventCC4', TaskId => v_NextTaskId);
  ...
  for rec in (select tsk.col_id as col_id, te.col_id as TaskEventId, te.col_processorcode as col_processorcode,
                ...
                and lower(ts.col_code) = v_TaskState
                and lower(timt.col_code) = 'action'
                ORDER BY te.col_taskeventorder ASC)
  loop
    --CALL PROCESSOR FUNCTION HERE AND GET RETURN VALUE
    v_isValid := 1;
    --ONLY FUNCTIONS (NOT REGULAR RULES) ARE EXECUTED SYNCHRONOUSLY
    if (substr(rec.col_processorcode, 1, 2) = 'f_') then
      v_result := f_DBG_createDBGTrace(CaseId => v_CaseId, Location => 'DCM_processEventCC4 before invocation of validation processor',
                                       ...
                                       Rule => 'DCM_processEventCC4', TaskId => v_NextTaskId);
      ...
      for rec2 in (select col_paramcode as ParamCode, col_paramvalue as ParamValue from tbl_autoruleparamcc where col_taskeventccautoruleparmcc = (select col_id from tbl_taskeventcc where col_id = rec.TaskEventId))
      loop
      ...
      v_result := f_DBG_createDBGTrace(CaseId => v_CaseId, Location => 'DCM_processEventCC4 after invocation of validation processor',
