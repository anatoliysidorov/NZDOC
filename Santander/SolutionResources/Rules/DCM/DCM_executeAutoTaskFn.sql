...
begin
  ...
  begin
    for rec in
      (select tsk.col_id as TaskId, tsk.col_id2, tsk.col_parentid, tsk.col_taskorder,
         tsk.col_casetask,tsk.col_tw_workitemtask, tsk.col_name as TaskName,
	     ...
	     twi.col_activity
         from tbl_task tsk
         inner join tbl_dict_executionmethod dem on tsk.col_taskdict_executionmethod = dem.col_id
         inner join tbl_dict_tasksystype dtt on tsk.col_taskdict_tasksystype = dtt.col_id
    ...
    loop
      ...
      update tbl_tw_workitem set col_activity = v_stateInProcess, col_tw_workitemprevtaskstate = col_tw_workitemdict_taskstate, col_tw_workitemdict_taskstate = (select col_id from tbl_dict_taskstate where col_activity = v_stateInProcess)
        where col_id = (select col_tw_workitemtask from tbl_task where col_id = rec.TaskId);
      --SET TASK DATE EVENT
      v_result := f_DCM_createTaskDateEvent (Name => 'DATE_TASK_IN_PROCESS', TaskId => rec.TaskId);
      --ADD TASK PROCESSOR TO EVENT QUEUE
      v_queueParams := f_UTIL_getJSONAutoTask(rec.TaskId);
