...
begin
  v_TaskId := :TaskId;
  begin
    select col_processorname into v_ProcessorName from tbl_task where col_id = v_TaskId;
    exception
...
END IF;	
  ...
  if v_count = 1 then
    ...
    execute immediate v_query using out v_result, out v_SessionId, v_TaskId;
  end if;
