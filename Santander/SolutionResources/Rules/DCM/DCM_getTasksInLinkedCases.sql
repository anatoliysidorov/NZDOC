select vtsk.id as id,vtsk.case_id as Case_Id,vtsk.childcaseid as ChildCaseId,vtsk.childtaskid as ChildTaskId,level,vtsk.name as name,vtsk.taskid as taskid,vtsk.taskext_id as TaskExt_Id,
       vtsk.description as description,vtsk.enabled as enabled,vtsk.icon as icon,vtsk.leaf as leaf,vtsk.required as required,vtsk.taskorder as taskorder,vtsk.parentid as parentid,
       vtsk.CaseSysType_Id as CaseSysType_Id,vtsk.executionmethod_id as executionmethod_id,vtsk.executionmethod_name as executionmethod_name,vtsk.executionmethod_code as executionmethod_code,
       vtsk.workitem_id as workitem_id,vtsk.workitem_workflow as workitem_workflow,vtsk.workitem_activity as workitem_activity,vtsk.workitem_activity_name as workitem_activity_name,
       vtsk.workbasket_id as workbasket_id,vtsk.workbasket_name as workbasket_name,vtsk.workbaskettype_id as workbaskettype_id,vtsk.workbaskettype_name as workbaskettype_name,
       vtsk.workbaskettype_code as workbaskettype_code,vtsk.caseworker_id as caseworker_id,vtsk.caseworker_name as caseworker_name,vtsk.tasksystypeid as tasksystypeid,vtsk.procedureid as procedureid,
       vtsk.PageCode as PageCode,vtsk.TaskState_Name as TaskState_Name,vtsk.TaskState_Code as TaskState_Code,vtsk.ResolutionCode_Id as ResolutionCode_Id,vtsk.ResolutionCode_Name as ResolutionCode_Name,
       vtsk.ResolutionCode_Code as ResolutionCode_Code,vtsk.ResolutionDescription as ResolutionDescription,vtsk.ManualWorkDuration as ManualWorkDuration,vtsk.ManualDateResolved as ManualDateResolved,
       vtsk.PERM_CASETYPE_VIEW as PERM_CASETYPE_VIEW,vtsk.AssignorName as AssignorName,vtsk.AssigneeName as AssigneeName,vtsk.NextSlaDateTime as NextSlaDateTime,vtsk.nextslaeventtypename as NextSlaEventTypename,
       vtsk.nextslaeventlevelname as NextSlaEventLevelName,vtsk.PrevSlaDateTime as PrevSlaDateTime,vtsk.prevslaeventtypename as PrevSlaEventTypename,vtsk.prevslaeventlevelname as PrevSlaEventLevelName
from   
(select ptsk.id as id, ptsk.case_id as Case_Id, ptsk.taskext_id as TaskExt_Id, ptsk.description as description, ptsk.enabled as enabled, ptsk.icon as icon,
        ...
        ptsk.ResolutionDescription as ResolutionDescription, ptsk.ManualWorkDuration as ManualWorkDuration, ptsk.ManualDateResolved as ManualDateResolved,
        ...
        case when f_dcm_getTaskClosedState2(StateConfigId => (select nvl(tst.col_stateconfigtasksystype,0)
            ...
            then ptsk.nextsladatetime else null end as NextSlaDateTime,
        ...
        case when f_dcm_getTaskClosedState2(StateConfigId => (select nvl(tst.col_stateconfigtasksystype,0)
                                                             ...
                                                             inner join tbl_task tsk on tst.col_id = tsk.col_taskdict_tasksystype
                                                             where tsk.col_id = ptsk.Id)) <> ptsk.workitem_activity
            then ptsk.prevsladatetime else null end as PrevSlaDateTime,
        ptsk.prevslaeventtypename                         AS PrevSlaEventTypename, 
        ptsk.prevslaeventlevelname                        AS PrevSlaEventLevelName,
        ccs.col_id as ChildCaseId, ctsk.col_id as ChildTaskId
        ...
        where ptsk.Case_Id in 
        (select :Case_Id from dual union select col_caselinkchildcase from tbl_caselink connect by prior col_caselinkchildcase = col_caselinkparentcase start with col_caselinkparentcase = :Case_Id)) vtsk
connect by prior vtsk.ChildTaskId = vtsk.Id or prior vtsk.id = vtsk.parentid
start with vtsk.parentid = 0 and vtsk.Case_Id = :Case_Id
