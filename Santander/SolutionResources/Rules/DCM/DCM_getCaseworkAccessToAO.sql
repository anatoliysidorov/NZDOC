...
nvl(s8.AccessObjectId, (select col_id from tbl_ac_accessobject where lower(col_code) = lower(:AccessObjectCode))) as AccessObjectId, s9.PermissionId as PermissionId,
...
from
(select col_id as PermissionId, col_defaultACL as DefaultACL, col_orderACL as OrderACL from tbl_ac_permission where col_id =
(select col_id from tbl_ac_permission where lower(col_code) = lower(:PermissionCode) and col_permissionaccessobjtype = (select col_accessobjaccessobjtype from tbl_ac_accessobject where lower(col_code) = lower(:AccessObjectCode)))) s9
inner join
...
and (case when aot.col_code = 'CASE_BUSINESS_OBJECT' and acl.col_processorcode is not null then f_dcm_invokeaclcaseproc(CaseId => :CaseId, ProcessorName => acl.col_processorcode)
          when aot.col_code = 'TASK_BUSINESS_OBJECT' and acl.col_processorcode is not null then f_dcm_invokeacltaskproc(ProcessorName => acl.col_processorcode, TaskId => :TaskId)
