...
BEGIN
  ...
  IF v_parentTask > 0 THEN
    SELECT v_countTasks + COUNT(col_id) INTO v_countTasks 
    FROM tbl_Task 
    WHERE NVL(COL_ISHIDDEN,0)=0
    START WITH col_id = v_parentTask CONNECT BY PRIOR col_id = COL_PARENTID;
  ELSE
    v_countTasks := 0;
  END IF;
  ...
  */
  ...
  IF v_ExternalPartyId > 0 THEN
    SELECT COUNT(DISTINCT c.col_id)
     ...
     RIGHT JOIN TBL_CASE c
        ON c.COL_ID = cp.COL_CASEPARTYCASE
     ...
     WHERE cp.COL_CASEPARTYEXTERNALPARTY = v_ExternalPartyId
        OR c.col_caseppl_workbasket = (SELECT col_id FROM tbl_ppl_workbasket WHERE COL_WORKBASKETEXTERNALPARTY = v_ExternalPartyId);
  END IF;
  ...
  --CREATE OUTPUT
  OPEN :ITEMS FOR
    SELECT v_countTasks            AS COUNTTASKS,
