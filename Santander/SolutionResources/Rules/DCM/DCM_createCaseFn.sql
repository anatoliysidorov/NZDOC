...
begin
  ...
  v_result := f_DBG_createDBGTrace(CaseId => v_col_CaseId, Location => 'DCM_createCaseFn begin', Message => 'Case is about to be created', Rule => 'DCM_createCaseFn', TaskId => null);
  begin
  ...
  v_DebugSession := f_DBG_createDBGSession(CaseId => v_col_CaseId, CaseTypeId => v_CaseSysTypeId, ProcedureId => v_col_ProcedureId);
  v_result := f_DBG_createDBGTrace(CaseId => v_col_CaseId, Location => 'DCM_createCaseFn end', Message => 'Case ' || to_char(v_col_CaseId) || ' is created', Rule => 'DCM_createCaseFn', TaskId => null);
