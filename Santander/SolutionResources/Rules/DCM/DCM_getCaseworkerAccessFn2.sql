...
s10.CaseworkerAccessSubjectId as CaseworkerAccessSubjectId, s10.CaseworkerAccessSubjectCode as CaseworkerAccessSubjectCode, s10.CaseworkerAccessSubjectName as CaseworkerAccessSubjectName, s10.Type as CaseworkerType,
...
cwas.col_id as CaseworkerAccessSubjectId, cwas.col_code as CaseworkerAccessSubjectCode, cwas.col_name as CaseworkerAccessSubjectName, cwas.col_type as Type
...
inner join tbl_ppl_caseworker cw on ep.userid = cw.col_userid
where cw.col_id = :p_CaseworkerId
...
and (case when aot.col_code = 'CASE_BUSINESS_OBJECT' and acl.col_processorcode is not null then f_dcm_invokeaclcaseproc(CaseId => :p_CaseId, ProcessorName => acl.col_processorcode)
          when aot.col_code = 'TASK_BUSINESS_OBJECT' and acl.col_processorcode is not null then f_dcm_invokeacltaskproc(ProcessorName => acl.col_processorcode, TaskId => :p_TaskId)
...
where ((s7.PeopleType in ('CASEWORKER', 'BUSINESSROLE', 'TEAM', 'SKILL')
  and s7.CaseworkerId = :p_CaseworkerId) or
      (s7.PeopleType = 'EXTERNALPARTY' and s7.CaseworkerId = :p_CaseworkerId))
