...
s9.CaseTypeId as CaseTypeId, s9.CaseTypeCode as CaseTypeCode, s9.CaseTypeName as CaseTypeName,
s9.TaskTypeId as TaskTypeId, s9.TaskTypeCode as TaskTypeCode, s9.TaskTypeName as TaskTypeName,
s9.CaseStateId as CaseStateId, s9.CaseStateCode as CaseStateCode, s9.CaseStateName as CaseStateName,
s9.UIElementId as UIElementId, s9.UIElementCode as UIElementCode, s9.UIElementName as UIElementName,
...
cst.col_id as CaseTypeId, cst.col_code as CaseTypeCode, cst.col_name as CaseTypeName,
tst.col_id as TaskTypeId, tst.col_code as TaskTypeCode, tst.col_name as TaskTypeName,
dcs.col_id as CaseStateid, dcs.col_code as CaseStateCode, dcs.col_name as CaseStateName,
uie.col_id as UIElementId, uie.col_code as UIElementCode, uie.col_name as UIElementName
...
) s10 on s10.CaseworkerId = (select id from vw_ppl_activecaseworkersusers where accode = sys_context('CLIENTCONTEXT', 'AccessSubject'))
left join
...
where s7.CaseworkerId = (select id from vw_ppl_activecaseworkersusers where accode = sys_context('CLIENTCONTEXT', 'AccessSubject'))
...
where lower(s9.accessobjecttypecode) = (case when :p_AccessObjectTypeCode is null then lower(s9.accessobjecttypecode) else cast(lower(:p_AccessObjectTypeCode) as nvarchar2(255)) end)
and lower(s9.permissioncode) = (case when :p_PermissionCode is null then lower(s9.permissioncode) else cast(lower(:p_PermissionCode) as nvarchar2(255)) end)
