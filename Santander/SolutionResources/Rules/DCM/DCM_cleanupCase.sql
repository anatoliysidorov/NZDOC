...
begin
  ...
  delete from (
  select *
  from tbl_doc_documentversion where 
  col_docversiondocid in (select col_doccasedocument from tbl_doc_doccase where col_doccasecase = v_CaseId)
  );

  delete from (
  select * from tbl_doc_document
  where col_id in (select col_doccasedocument from tbl_doc_doccase where col_doccasecase = v_CaseId)
  );
