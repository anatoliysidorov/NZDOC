select vtsk.id as id,
       ...
       vtsk.RowNumber as TaskRowNumber,
       vtsk.depth as Depth,
       vtsk.name as name,
...
from
(with s11 as (select col_id as TaskId, rownum as RowNumber
from tbl_task
where col_casetask = :Case_Id
start with col_parentid = 0
connect by prior col_id = col_parentid
order siblings by col_taskorder)
select ptsk.id as id, s11.RowNumber as RowNumber, ptsk.case_id as Case_Id, ptsk.taskext_id as TaskExt_Id,
        DBMS_LOB.SUBSTR(ptsk.description, 1, 200) as description, ptsk.enabled as enabled, ptsk.icon as icon,
        ptsk.leaf as leaf, ptsk.name as name, ptsk.required as required, ptsk.depth as Depth,
...
inner join s11 on ptsk.id = s11.TaskId
