...
begin
  ...
  v_result := f_DBG_createDBGTrace(CaseId => v_CaseId, Location => 'DCM_processEventCC7 before validation begins',
                                   Message => 'Task ' || to_char(v_NextTaskId) || ' before validation event processing', Rule => 'DCM_processEventCC7', TaskId => v_NextTaskId);
  begin
  ...
  loop
    ...
    if nvl(rec.IsFunction, 0) = 0 then
      v_result := f_DBG_createDBGTrace(CaseId => v_CaseId, Location => 'DCM_processEventCC7 before invocation of validation processor',
                                       ...
                                       Rule => 'DCM_processEventCC7', TaskId => v_NextTaskId);
      v_queueParams := f_UTIL_getJSONAutoRulePrmCC(rec.taskeventid);
      v_result := f_UTIL_addToQueueFn(RuleCode => rec.col_processorcode, Parameters => v_queueParams);
      v_result := f_DBG_createDBGTrace(CaseId => v_CaseId, Location => 'DCM_processEventCC7 after invocation of validation processor',
  ...
  v_result := f_DBG_createDBGTrace(CaseId => v_CaseId, Location => 'DCM_processEventCC7 after validation',
                                   Message => 'Task ' || to_char(v_NextTaskId) || ' after validation event processing', Rule => 'DCM_processEventCC7', TaskId => v_NextTaskId);
  :Message := v_Message;
  :IsValid := v_isValid;
  return v_isValid;
end;