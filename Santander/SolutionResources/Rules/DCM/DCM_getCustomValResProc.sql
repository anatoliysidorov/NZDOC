declare
  ...
  v_customvalresproc nvarchar2(255);
  v_result number;
begin
  ...
  if lower(v_ProcInCaseTypeCode) = 'dedicated_single' then
    ...
    begin
      select col_customvalresultprocessor into v_customvalresproc from tbl_dict_casesystype where col_id = v_CaseTypeId;
      exception
      when NO_DATA_FOUND then
        v_customvalresproc := null;
    end;
    ...
    begin
      select col_customvalresultprocessor into v_customvalresproc from tbl_procedure where col_id = v_ProcedureId;
      exception
      when NO_DATA_FOUND then
      v_customvalresproc := null;
    end;
  ...
  :CustomValResProc := v_customvalresproc;
end;