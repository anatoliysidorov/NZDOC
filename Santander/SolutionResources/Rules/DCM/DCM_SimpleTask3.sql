SELECT
          ...
          /*--SYSTEM TYPE AND PROCEDURE*/
           ...
           CASE
           WHEN dts.col_isfinish=1 THEN null
           ELSE
           ...
           (cast((select max(de2.col_datevalue) from tbl_dateevent de2 where de2.col_dateeventtask = t.col_id AND de2.col_dateevent_dateeventtype = gse.col_slaevent_dateeventtype) +
             ...
             (case when gse.col_intervalym is not null then to_yminterval(gse.col_intervalym) else to_yminterval('0-0') end) as timestamp))
           END AS GoalSlaDateTime,
           ...
           (cast((select max(de2.col_datevalue) from tbl_dateevent de2 where de2.col_dateeventtask = t.col_id AND de2.col_dateevent_dateeventtype = dse.col_slaevent_dateeventtype) +
             ...
             (case when dse.col_intervalym is not null then to_yminterval(dse.col_intervalym) else to_yminterval('0-0') end) as timestamp))
           END AS DLineSlaDateTime,

           /*--SLA EVENT DATA SECTION END*/
