...
begin
  ...
  if v_caseTypeId is not null then
    update tbl_case set col_casedict_casesystype = v_caseTypeId where col_id = v_createdCaseId;
  end if;
  ...
  if v_customdataprocessor is not null then
    ...
    v_createdCaseExtId := f_dcm_invokeCaseCusDataProc(CaseId => v_caseId, Input => v_customdata, ProcessorName => v_customdataprocessor);
  else
