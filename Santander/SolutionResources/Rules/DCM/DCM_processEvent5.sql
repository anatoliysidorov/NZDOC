...
begin
  ...
  v_result := f_DBG_createDBGTrace(CaseId => v_CaseId, Location => 'DCM_processEvent5 before validation begins',
                                   Message => 'Task ' || to_char(v_NextTaskId) || ' before validation event processing', Rule => 'DCM_processEvent5', TaskId => v_NextTaskId);
  ...
  loop
    ...
    v_result := f_DBG_createDBGTrace(CaseId => v_CaseId, Location => 'DCM_processEvent5 before invocation of validation processor',
                                     ...
                                     Rule => 'DCM_processEvent5', TaskId => v_NextTaskId);
    ...
    v_result := f_DBG_createDBGTrace(CaseId => v_CaseId, Location => 'DCM_processEvent5 after invocation of validation processor',
  ...
  v_result := f_DBG_createDBGTrace(CaseId => v_CaseId, Location => 'DCM_processEvent5 after validation',
                                   Message => 'Task ' || to_char(v_NextTaskId) || ' after validation event processing', Rule => 'DCM_processEvent5', TaskId => v_NextTaskId);
  :Message := v_Message;
  :IsValid := v_isValid;
  return v_isValid;
end;
