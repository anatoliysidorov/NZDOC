WITH s01 AS (
SELECT
max((CASE WHEN subs2.datevalue IS NOT NULL THEN subs2.datevalue ELSE SYSDATE END)) AS dateeventvalue,tsk.col_id AS taskid,se.col_id AS slaeventid
FROM tbl_task tsk
...
WHERE dec.col_datename = 'DATE_TASK_CLOSED') subs2 ON tsk.col_id = subs2.dateeventtask
WHERE cs.col_id = :CaseId
group by tsk.col_id, se.col_id
),
s11 AS (
...
SELECT se.col_id AS slaeventid,tsk.col_id AS taskid,tsk.col_name AS taskname,tsk.col_taskid AS tasktitle,tsk.col_casetask AS caseid,tsk.col_parentid AS taskparentid,
       tsk.col_tw_workitemtask AS taskworkitemid,setp.col_id AS slaeventtypeid,setp.col_code AS slaeventtypecode,setp.col_name AS slaeventtypename,
       ...
       sel.col_id AS slaeventlevelid,sel.col_code AS slaeventlevelcode,sel.col_name AS slaeventcodename,det.col_id AS dateeventtypeid,
...
SELECT s1.slaeventid AS slaeventid,s1.taskid AS taskid,s1.taskname AS taskname,s1.tasktitle AS tasktitle,s1.caseid AS caseid,s1.taskparentid AS taskparentid,s1.taskworkitemid AS taskworkitemid,
       ...
       s1.slaeventtypeintervalds AS slaeventtypeintervalds,s1.slaeventtypeintervalym AS slaeventtypeintervalym,s1.slaeventintervalds AS slaeventintervalds,s1.slaeventintervalym AS slaeventintervalym,
       s1.slaeventlevelid AS slaeventlevelid,s1.slaeventlevelcode AS slaeventlevelcode,
       s1.slaeventcodename AS slaeventlevelname,
...
(SELECT slaeventid,taskid,taskname,tasktitle,caseid,taskparentid,taskworkitemid,sladatetime,sladaysleft,trunc(months_between(sladatetime,SYSDATE)) AS trancated,slaeventtypeid,
        slaeventtypecode,slaeventtypename,slaeventtypeintervalds,slaeventtypeintervalym,slaeventintervalds,slaeventintervalym,slaeventlevelid,slaeventlevelcode,slaeventcodename,dateeventtypeid,dateeventtypecode,
...
--ACTUAL SELECT STARTS BELOW
 SELECT s31.slaeventid AS id,s31.slaeventid AS slaeventid,s31.taskid,s31.taskname,s31.tasktitle,s31.caseid,s31.taskparentid,s31.taskworkitemid,
        ...
        slaeventtypeid,slaeventtypecode,slaeventtypename,slaeventintervalds,slaeventintervalym,slaeventtypeintervalds,slaeventtypeintervalym,slaeventlevelid,slaeventlevelcode,slaeventlevelname,
