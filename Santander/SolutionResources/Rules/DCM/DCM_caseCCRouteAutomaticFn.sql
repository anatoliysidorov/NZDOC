...
begin
  ...
  v_result := f_DBG_createDBGTrace(CaseId => v_CaseId, Location => 'DCM_caseCCRouteAutomaticFn begin', Message => 'Before routing case ' || to_char(v_CaseId), Rule => 'DCM_caseCCRouteAutomaticFn', TaskId => null);
  begin
  ...
  v_transition := f_DCM_getNextCaseCCTransAuto(CaseActivity => v_state, CaseId => v_CaseId);
  if v_transition is null then
    return -1;
  end if;
  v_state := f_DCM_getNextCaseCCActtyAuto(CaseActivity => v_state, CaseId => v_CaseId);
  ...
  begin
    ...
    (select cs.col_id as CaseId, cs.col_caseid as CaseTitle, dcst.col_code as CaseSysType,
       ...
       null as CaseDescription, mcsi.col_id as CaseStateInitId, mcsi.col_processorcode as CaseStateInitProc, mcsi.col_assignprocessorcode as CaseStateInitAssignProc,
...
*/                
  ...
  v_result := f_DBG_createDBGTrace(CaseId => v_CaseId, Location => 'Before validation', Message => 'Case ' || to_char(v_CaseId) || ' before validation', Rule => 'DCM_caseCCRouteAutomaticFn', TaskId => null);
  v_result := f_DCM_processCaseCCEvent2(CaseId => v_CaseId, EventState => v_target, EventType => 'before', IsValid => v_IsValid);
  v_result := f_DBG_createDBGTrace(CaseId => v_CaseId, Location => 'Before validation', Message => 'Case ' || to_char(v_CaseId) || ' after validation with validation result: ' || to_char(v_IsValid),
                                   Rule => 'DCM_caseCCRouteAutomaticFn', TaskId => null);
  if v_IsValid = 1 then
    v_resolutionAssigned := 0;
    begin
      --SET CASE rec.CaseId IN CURSOR rec TO STATE v_target
      update tbl_cw_workitemcc set col_activity = v_target, col_cw_workitemccprevcasest = col_cw_workitemccdict_casest, col_cw_workitemccdict_casest = (select col_id from tbl_dict_casestate where col_activity = v_target
      and nvl(col_stateconfigcasestate,0) = (select nvl(col_stateconfigcasesystype,0) from tbl_dict_casesystype where col_id =
      (select col_caseccdict_casesystype from tbl_casecc where col_id = v_CaseId)))
        where col_id = (select col_cw_workitemcccasecc from tbl_casecc where col_id = v_CaseId);
      update tbl_casecc set col_activity = v_target, col_caseccdict_casestate = (select col_id from tbl_dict_casestate where col_activity = v_target
      and nvl(col_stateconfigcasestate,0) = (select nvl(col_stateconfigcasesystype,0) from tbl_dict_casesystype where col_id =
      (select col_caseccdict_casesystype from tbl_casecc where col_id = v_CaseId)))
        where col_id = v_CaseId;
      v_result := f_DBG_createDBGTrace(CaseId => v_CaseId, Location => 'After case routing', Message => 'Case ' || to_char(v_CaseId) || ' after routing to state ' || v_target, Rule => 'DCM_caseCCRouteAutomaticFn', TaskId => null);
      if v_target = v_stateAssigned then
        ...
        v_result := f_DBG_createDBGTrace(CaseId => v_CaseId, Location => 'After case routing', Message => 'Case ' || to_char(v_CaseId) || ' after assignment', Rule => 'DCM_caseCCRouteAutomaticFn', TaskId => null);
      end if;
      for rec2 in
      ...
      loop
        if rec2.casestatesetupcode = 'DATEASSIGNED' then
          ...
          if rec2.ForcedNull = 1 then
            ...
            update tbl_casecc set col_caseccppl_workbasket = col_caseccppl_workbasket where col_id = v_CaseId;
          end if;
        elsif rec2.casestatesetupcode = 'RESOLUTION' then
          update tbl_casecc set col_stp_resolutioncodecasecc = col_stp_resolutioncodecasecc where col_id = v_CaseId;
      ...
      v_result := f_DBG_createDBGTrace(CaseId => v_CaseId, Location => 'After case date events added', Message => 'Case ' || to_char(v_CaseId) || ' after date events added', Rule => 'DCM_caseCCRouteAutomaticFn', TaskId => null);
      exception
        when NO_DATA_FOUND then
          return -1;
    end;
    ...
    v_result := f_DBG_createDBGTrace(CaseId => v_CaseId, Location => 'After notification created', Message => 'Case ' || to_char(v_CaseId) || ' notification created', Rule => 'DCM_caseCCRouteAutomaticFn', TaskId => null);
    --AFTER TRANSITION ACTION SYNCHRONOUS EVENTS
    v_result := f_DCM_processCaseCCEvent3(CaseId => v_CaseId, EventState => v_state, IsValid => v_IsValid);
    ...
    if v_resolutionAssigned = 0 then
      v_result := f_DCM_createCaseHistoryCC (MessageCode => 'CaseStateRouted', CaseId => v_CaseId, IsSystem => 1);
    elsif v_resolutionAssigned = 1 then
      v_result := f_DCM_createCaseHistoryCC (MessageCode => 'CaseStateRoutedWithResolution', CaseId => v_CaseId, IsSystem => 1);
    end if;
    v_result := f_DBG_createDBGTrace(CaseId => v_CaseId, Location => 'After case history created', Message => 'Case ' || to_char(v_CaseId) || ' history created created', Rule => 'DCM_caseCCRouteAutomaticFn', TaskId => null);
    ...
    v_result := f_DBG_createDBGTrace(CaseId => v_CaseId, Location => 'After statistics calculated', Message => 'Case ' || to_char(v_CaseId) || ' after statistics calculated', Rule => 'DCM_caseCCRouteAutomaticFn', TaskId => null);
    if v_target = f_dcm_getCaseClosedState2(f_dcm_getcaseconfigid(CaseId => v_CaseId)) then
      ...
      v_result := f_DBG_createDBGTrace(CaseId => v_CaseId, Location => 'After statistics updated', Message => 'Case ' || to_char(v_CaseId) || ' after statistics updated', Rule => 'DCM_caseCCRouteAutomaticFn', TaskId => null);
    end if;
  else
    return -1;
  end if;
