...
begin
  ...
  begin
    select count(*) into v_count from tbl_slaevent se
      ...
      inner join tbl_case cs on se.col_slaeventcase = cs.col_id
      where se.col_slaeventcase = v_CaseId
  ...
  if v_count > 0 then
    begin
      ...
      select count(*) into v_count2
        ...
        inner join tbl_dict_casesystype cst on cse.col_casedict_casesystype = cst.col_id
        inner join tbl_dict_casestate cs on cwi.col_cw_workitemdict_casestate = cs.col_id
        --DATE WHEN SLA STARTED
        inner join tbl_dateevent des on cse.col_id = des.col_dateeventcase
        inner join tbl_dict_dateeventtype dets on des.col_dateevent_dateeventtype = dets.col_id
        --DATE WHEN SLA ENDED
        inner join tbl_dateevent dee on cse.col_id = dee.col_dateeventcase
        ...
        and cse.col_id not in (select col_slaeventcase
                                 from tbl_slaevent se
                                 inner join tbl_dict_slaeventtype setp on se.col_slaeventdict_slaeventtype = setp.col_id and setp.col_code = 'GOAL'
                                 where col_slaeventcase = cse.col_id
        ...
        and dee.col_datevalue between trunc(sysdate, 'MM') and trunc(add_months(sysdate, 1), 'MM');
      exception
        when NO_DATA_FOUND then
          v_count2 := 0;
    end;
  ...
  if v_count > 0 then
    begin
      ...
      select count(*) into v_count2
        ...
        inner join tbl_dict_dateeventtype dets on des.col_dateevent_dateeventtype = dets.col_id
        --DATE WHEN SLA ENDED
        inner join tbl_dateevent dee on cse.col_id = dee.col_dateeventtask
        inner join tbl_dict_dateeventtype dete on dee.col_dateevent_dateeventtype = dete.col_id
        where cse.col_id = v_CaseId
        and (cse.col_statupdated is null or cse.col_statupdated = 0)
        and cs.col_activity = v_stateCaseComplete
        --INCLUDE CASES THAT DO NOT MEET AT LEAST ONE GOAL
        and cse.col_id in (select col_slaeventcase
                             from tbl_slaevent se
                             inner join tbl_dict_slaeventtype setp on se.col_slaeventdict_slaeventtype = setp.col_id and setp.col_code = 'GOAL'
                             where col_slaeventcase = cse.col_id
                             and (des.col_datevalue + (case when se.col_intervalds is not null then to_dsinterval(se.col_intervalds) else interval '0' day end)
  ...
  v_CaseComplete := 1;
  begin
    select cst.col_id, cwu.Id, to_char(trunc(dee.col_datevalue, 'MM')), (dee.col_datevalue - des.col_datevalue) * 24 * 60
  ...
  if (v_CaseComplete is not null) and (v_CaseSysType is not null) and (v_CaseWorkerId is not null) and (v_YearMonth is not null) then
    begin
      select col_id into v_id
        ...
        where col_statcasemonthcasesystype = v_CaseSysType
        and col_statcasemonthcaseworker = v_Caseworkerid
    ...
    if v_id is not null then
      begin
        select col_avginprocess, col_totalinprocess, col_totalclosed into v_AvgInProcess, v_TotalInProcess, v_TotalClosed from tbl_statcasemonth where col_id = v_id;
        exception
          when NO_DATA_FOUND then
            v_AvgInProcess := 0;
            v_TotalInProcess := 0;
      ...
      insert into tbl_statcasemonth (col_statcasemonthcaseworker, col_statcasemonthcasesystype, col_yearmonth, col_totalclosed, col_meetgoal, col_nomeetgoal, col_meetdeadline, col_nomeetdeadline, col_avginprocess, col_totalinprocess)
