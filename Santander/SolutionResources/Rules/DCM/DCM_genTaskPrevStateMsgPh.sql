...
BEGIN 
  ...
  v_CSisInCache :=f_DCM_CSisTaskInCache(TaskID => v_taskid);
    ...
    FROM   tbl_taskCC tse 
     ...
     inner join tbl_dict_taskstate ts ON twi.COL_TW_WORKITEMCCPREVTASKST = ts.col_id 
  ...
  IF (v_inCache = 0) AND (v_CSisInCache=0) THEN
    ...
    FROM   tbl_task tse 
     ...
     inner join tbl_dict_taskstate ts ON twi.COL_TW_WORKITEMPREVTASKSTATE = ts.col_id 
    WHERE  tse.col_id = v_taskid;
	END IF;
