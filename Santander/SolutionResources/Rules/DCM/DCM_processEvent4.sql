...
begin
  ...
  v_result := f_DBG_createDBGTrace(CaseId => v_CaseId, Location => 'DCM_processEvent4 before validation begins',
                                   Message => 'Task ' || to_char(v_NextTaskId) || ' before validation event processing', Rule => 'DCM_processEvent4', TaskId => v_NextTaskId);
  ...
  loop
    ...
    if (substr(rec.col_processorcode, 1, 2) = 'f_') then
      v_result := f_DBG_createDBGTrace(CaseId => v_CaseId, Location => 'DCM_processEvent4 before invocation of validation processor',
                                       ...
                                       Rule => 'DCM_processEvent4', TaskId => v_NextTaskId);
      ...
      v_result := f_DBG_createDBGTrace(CaseId => v_CaseId, Location => 'DCM_processEvent4 after invocation of validation processor',
