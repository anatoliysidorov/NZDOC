...
inner join tbl_dateevent des on ((case when des.col_dateeventcase is not null then (cs.col_id) else (tsk.col_id) end) = 
...
inner join tbl_dateevent de2 on ((case when de2.col_dateeventcase is not null then (cs2.col_id) else (tsk2.col_id) end) = 
                                  case when de2.col_dateeventcase is not null then (de2.col_dateeventcase) else (de2.col_dateeventtask) end)
                                  /*tsk2.col_id = de2.col_dateeventtask*/
                                  and se2.col_slaevent_dateeventtype = de2.col_dateevent_dateeventtype
inner join s02 on tsk2.col_id = s02.TaskId) s
where s.RowNumber = 1)
--------------------------------------------------------------------------------------------------------------------------------------------------------------------------
select NextSlaEventId as Id, NextSlaEventId as NextSlaEventId, NextTaskId, NextTaskName, NextTaskTitle,
...
(case when (NextSlaDateTime - sysdate > to_dsinterval('0 0' || ':' || '0' || ':' || '0')) then 'Upcoming' else 'Passed' end) as SlaPassed,
/*NextSlaDaysLeft,*/
