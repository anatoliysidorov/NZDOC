...
begin
  ...
  v_result := f_DBG_createDBGTrace(CaseId => v_CaseId, Location => 'DCM_routeCaseCCTasksCCFn3 begin', Message => 'Before routing tasks in case ' || to_char(v_CaseId), Rule => 'DCM_routeCaseCCTasksCCFn3', TaskId => null);
  ...
  for rec in (
              ...
              cast(tsk.col_description as nvarchar2(2000)) as TaskDescription, twi.col_id as twId, twi.col_activity as twActivity, dts.col_activity as twTargetActivity,
              ...
              inner join tbl_dict_tasktransition dtt on dtt.col_sourcetasktranstaskstate = twi.col_tw_workitemccdict_taskst and dtt.col_targettasktranstaskstate = dts.col_id
              ...
              and dts.col_activity in (select null from dual union select NextActivity from table(f_DCM_getNextActivityListCC(TaskId => tsk.col_id)))
              ...
              and tsk.col_id not in (select tsic.col_map_taskstateinitcctaskcc
                                         ...
                                         inner join tbl_dict_tasktransition dttc on dttc.col_sourcetasktranstaskstate = twic.col_tw_workitemccdict_taskst
                                              and dttc.col_targettasktranstaskstate = dtsc.col_id
                                              and nvl(dttc.col_manualonly, 0) = 0
                                         where tskp.col_casecctaskcc = v_CaseId
                                         --FILTER CHILD TASK INITIATION RECORDS TO STATE 'root_TSK_Status_STARTED'
                                         and dtsc.col_activity in (select null from dual union select NextActivity from table(f_DCM_getNextActivityListCC(TaskId => tskc.col_id)))
                ...
                and ((select count(*)
                        from
                          (select tsic.col_map_taskstateinitcctaskcc, tskc.col_id as TaskId
                             from tbl_taskdependencycc td
                             --JOIN TASK INITIATION RECORDS FOR PARENT TASK
                             inner join tbl_map_taskstateinitcc tsip on td.col_taskdpprntcctaskstinitcc = tsip.col_id
                             --JOIN TASK INITIATION RECORDS FOR CHILD TASK
...
select tsk.col_id as TaskId, tsk.col_taskid as TaskBsId, tsk.col_name as Taskname, dtst.col_code as TaskSysType, tsk.col_type as TaskType,
                          tsk.col_dateassigned as TaskDateAssigned, tsk.col_dateclosed as TaskDateClosed,
                          tsk.col_depth as TaskDepth, cast(tsk.col_description as nvarchar2(2000)) as TaskDescription,
...
select tsk.col_id as TaskId, tsk.col_taskid as TaskBsId, tsk.col_name as Taskname, dtst.col_code as TaskSysType, tsk.col_type as TaskType,
       ...
       mtsi.col_id as TaskStateInitId, mtsi.col_processorcode as TaskStateInitProc, mtsi.col_assignprocessorcode as TaskStateInitAssignProc
                ...
                inner join tbl_dict_taskstate dts on mtsi.col_map_tskstinitcc_tskst = dts.col_id
                inner join tbl_dict_initmethod dim on mtsi.col_map_tskstinitcc_initmtd = dim.col_id
                inner join tbl_dict_tasktransition dtt on twi.col_tw_workitemccdict_taskst = dtt.col_sourcetasktranstaskstate and dts.col_id = dtt.col_targettasktranstaskstate
                                                       and nvl(dtt.col_manualonly, 0) = 0
                where tsk.col_casecctaskcc = v_CaseId
                --TASKS WITH STATUS 'root_TSK_Status_NEW' CAN BE STARTED
                and twi.col_activity in (select col_activity from tbl_dict_taskstate)
                ...
                and dts.col_activity in (select null from dual union select NextActivity from table(f_DCM_getNextActivityListCC(TaskId => tsk.col_id)))
                and (not exists (select col_taskdpchldcctaskstinitcc from tbl_taskdependencycc where col_taskdpchldcctaskstinitcc = mtsi.col_id)
                      or exists (select col_taskdpchldcctaskstinitcc from tbl_taskdependencycc where col_taskdpchldcctaskstinitcc = mtsi.col_id and col_type in ('FSC', 'FS'))
                     )
                and (tsk.col_id, mtsi.col_id) not in
                (select chldtsk.col_id as ChildTaskId, chldmtsi.col_id as ChildMtsiId
                from (select col_id, col_taskccstp_resolutioncode, col_tw_workitemcctaskcc, col_taskccdict_tasksystype, col_casecctaskcc from tbl_taskcc where col_casecctaskcc = v_CaseId) tsk
                ...
                inner join tbl_dict_taskstate dts on mtsi.col_map_tskstinitcc_tskst = dts.col_id
                inner join tbl_dict_initmethod dim on mtsi.col_map_tskstinitcc_initmtd = dim.col_id
                inner join tbl_taskdependencycc td on mtsi.col_id = td.col_taskdpprntcctaskstinitcc and td.col_type in ('FSC', 'FS', 'FSCLR')
                ...
                inner join tbl_taskcc chldtsk on chldmtsi.col_map_taskstateinitcctaskcc = chldtsk.col_id and chldtsk.col_casecctaskcc = v_CaseId
                ...
                inner join tbl_dict_tasktransition dtt on dtt.col_sourcetasktranstaskstate = chldtwi.col_tw_workitemccdict_taskst and dtt.col_targettasktranstaskstate = chlddts.col_id
                                                       and nvl(dtt.col_manualonly, 0) = 0
                ...
                and chlddts.col_activity in (select null from dual union select NextActivity from table(f_DCM_getNextActivityListCC(TaskId => chldtsk.col_id)))
                ...
                and (chldtsk.col_id, chldmtsi.col_id) not in
                (select chldtsk.col_id, chldmtsi.col_id
                from (select col_id, col_taskccstp_resolutioncode, col_tw_workitemcctaskcc, col_taskccdict_tasksystype, col_casecctaskcc from tbl_taskcc where col_casecctaskcc = v_CaseId) tsk
                                inner join tbl_tw_workitemcc twi on tsk.col_tw_workitemcctaskcc = twi.col_id
                inner join tbl_dict_tasksystype dtst on tsk.col_taskccdict_tasksystype = dtst.col_id
                inner join tbl_map_taskstateinitcc mtsi on tsk.col_id = mtsi.col_map_taskstateinitcctaskcc
                ...
                and chlddts.col_activity in (select null from dual union select NextActivity from table(f_DCM_getNextActivityListCC(TaskId => chldtsk.col_id)))
                and lower(chlddim.col_code) = 'automatic_rule'
                and ((twi.col_activity = dts.col_activity and mtsi.col_routedby is not null and mtsi.col_routeddate is not null)
                ...
                inner join tbl_dict_taskstate dts on mtsi.col_map_tskstinitcc_tskst = dts.col_id
                inner join tbl_dict_initmethod dim on mtsi.col_map_tskstinitcc_initmtd = dim.col_id
                inner join tbl_taskdependencycc td on mtsi.col_id = td.col_taskdpprntcctaskstinitcc and td.col_type not in ('FSC', 'FS', 'FSCLR')
                inner join tbl_map_taskstateinitcc chldmtsi on td.col_taskdpchldcctaskstinitcc = chldmtsi.col_id
                inner join tbl_dict_taskstate chlddts on chldmtsi.col_map_tskstinitcc_tskst = chlddts.col_id
                inner join tbl_taskcc chldtsk on chldmtsi.col_map_taskstateinitcctaskcc = chldtsk.col_id and chldtsk.col_casecctaskcc = v_CaseId
                inner join tbl_tw_workitemcc chldtwi on chldtsk.col_tw_workitemcctaskcc = chldtwi.col_id
                ...
                inner join tbl_dict_taskstate dts on mtsi.col_map_tskstinitcc_tskst = dts.col_id
                inner join tbl_dict_initmethod dim on mtsi.col_map_tskstinitcc_initmtd = dim.col_id
                inner join tbl_map_taskstateinitcc chldmtsi on mtsi.col_map_taskstateinitcctaskcc = chldmtsi.col_map_taskstateinitcctaskcc
                inner join tbl_dict_taskstate chlddts on chldmtsi.col_map_tskstinitcc_tskst = chlddts.col_id
                inner join tbl_taskcc chldtsk on chldmtsi.col_map_taskstateinitcctaskcc = chldtsk.col_id and chldtsk.col_casecctaskcc = v_CaseId
                inner join tbl_tw_workitemcc chldtwi on chldtsk.col_tw_workitemcctaskcc = chldtwi.col_id
                inner join tbl_dict_tasksystype chlddtst on chldtsk.col_taskccdict_tasksystype = chlddtst.col_id
                ...
                and ((twi.col_activity <> dts.col_activity)
                     and (case when chldmtsi.col_processorcode is not null then f_DCM_invokeTaskProcessor(chldmtsi.col_processorcode, chldmtsi.col_id)
                              else 1 end) <> 1))
union
select tsk.col_id as TaskId, tsk.col_taskid as TaskBsId, tsk.col_name as Taskname, dtst.col_code as TaskSysType, tsk.col_type as TaskType,
       tsk.col_dateassigned as TaskDateAssigned, tsk.col_dateclosed as TaskDateClosed,
       ...
       mtsi.col_id as TaskStateInitId, mtsi.col_processorcode as TaskStateInitProc, mtsi.col_assignprocessorcode as TaskStateInitAssignProc
                ...
                and lower(dim.col_code) = 'automatic_rule'
                and dts.col_activity in (select null from dual union select NextActivity from table(f_DCM_getNextActivityListCC(TaskId => tsk.col_id)))
                and (tsk.col_id, mtsi.col_id) in
                (select s1.ChildTaskId, s1.ChildMTSIId
                from
                (select chldtsk.col_id as ChildTaskId, tsk.col_id as ParentTaskId, td.col_id as TaskDependencyId,
                chlddts.col_activity as ChildTaskTargetActivity,
                mtsi.col_id as MTSIId, mtsi.col_routeddate as ParentTaskRouteddate, chldmtsi.col_id as ChildMTSIId,
                ...
                inner join tbl_dict_taskstate dts on mtsi.col_map_tskstinitcc_tskst = dts.col_id
                inner join tbl_dict_initmethod dim on mtsi.col_map_tskstinitcc_initmtd = dim.col_id
                inner join tbl_taskdependencycc td on mtsi.col_id = td.col_taskdpprntcctaskstinitcc and td.col_type in ('FSCLR')
                inner join tbl_map_taskstateinitcc chldmtsi on td.col_taskdpchldcctaskstinitcc = chldmtsi.col_id
                inner join tbl_dict_taskstate chlddts on chldmtsi.col_map_tskstinitcc_tskst = chlddts.col_id
                inner join tbl_taskcc chldtsk on chldmtsi.col_map_taskstateinitcctaskcc = chldtsk.col_id and chldtsk.col_casecctaskcc = v_CaseId
                inner join tbl_tw_workitemcc chldtwi on chldtsk.col_tw_workitemcctaskcc = chldtwi.col_id
    ...
    loop
      v_result := f_DCM_routeCaseCCTaskCCFn(CaseId => v_CaseId, TaskId => rec.TaskId, TaskStateInitAssignProc => rec.TaskStateInitAssignProc, TaskStateInitId => rec.TaskStateInitId, TwTargetActivity => rec.twTargetActivity);
      if v_result = -1 then
        return -1;
    ...
    for rec in (
       ...
       mtsi.col_id as TaskStateInitId, mtsi.col_processorcode as TaskStateInitProc, mtsi.col_assignprocessorcode as TaskStateInitAssignProc
                ...
                and lower(dim.col_code) = 'automatic_rule'
                and dts.col_activity in (select null from dual union select NextActivity from table(f_DCM_getNextActivityListCC(TaskId => tsk.col_id)))
                and exists (select col_taskdpchldcctaskstinitcc from tbl_taskdependencycc where col_taskdpchldcctaskstinitcc = mtsi.col_id and col_type = 'FSCA')
                and (tsk.col_id, mtsi.col_id) not in
                (select chldtsk.col_id as ChildTaskId, chldmtsi.col_id as ChildMtsiId
                from (select col_id, col_taskccstp_resolutioncode, col_tw_workitemcctaskcc, col_taskccdict_tasksystype, col_casecctaskcc from tbl_taskcc where col_casecctaskcc = v_CaseId) tsk
                left  join tbl_stp_resolutioncode src on tsk.col_taskccstp_resolutioncode = src.col_id
                ...
                inner join tbl_dict_taskstate dts on mtsi.col_map_tskstinitcc_tskst = dts.col_id
                inner join tbl_dict_initmethod dim on mtsi.col_map_tskstinitcc_initmtd = dim.col_id
                inner join tbl_taskdependencycc td on mtsi.col_id = td.col_taskdpprntcctaskstinitcc and td.col_type in ('FS','FSC','FSCLR','FSCA','FSCO','FSCEX')
                inner join tbl_map_taskstateinitcc chldmtsi on td.col_taskdpchldcctaskstinitcc = chldmtsi.col_id
                inner join tbl_dict_taskstate chlddts on chldmtsi.col_map_tskstinitcc_tskst = chlddts.col_id
                inner join tbl_taskcc chldtsk on chldmtsi.col_map_taskstateinitcctaskcc = chldtsk.col_id and chldtsk.col_casecctaskcc = v_CaseId
                inner join tbl_tw_workitemcc chldtwi on chldtsk.col_tw_workitemcctaskcc = chldtwi.col_id
                ...
                and ((twi.col_activity <> dts.col_activity)
                       or ((case
                           when td.col_processorcode is not null and twi.col_activity = dts.col_activity and mtsi.col_routedby is not null and mtsi.col_routeddate is not null
                                then f_DCM_invokeTaskProcessor2(td.col_processorcode, td.col_id)
                           when chldmtsi.col_processorcode is not null and twi.col_activity = dts.col_activity and mtsi.col_routedby is not null and mtsi.col_routeddate is not null
                                then f_DCM_invokeTaskProcessor(chldmtsi.col_processorcode, chldmtsi.col_id)
                           when twi.col_activity <> dts.col_activity and mtsi.col_routedby is null and mtsi.col_routeddate is null
                                then 0
                           end) <> 1))
                and (chldtsk.col_id, chldmtsi.col_id) not in
                (select chldtsk.col_id as ChildTaskId, chldmtsi.col_id as ChildMtsiId
                ...
                inner join tbl_dict_taskstate dts on mtsi.col_map_tskstinitcc_tskst = dts.col_id
                inner join tbl_dict_initmethod dim on mtsi.col_map_tskstinitcc_initmtd = dim.col_id
                inner join tbl_taskdependencycc td on mtsi.col_id = td.col_taskdpprntcctaskstinitcc and td.col_type in ('FSCA')
                inner join tbl_map_taskstateinitcc chldmtsi on td.col_taskdpchldcctaskstinitcc = chldmtsi.col_id
                inner join tbl_dict_taskstate chlddts on chldmtsi.col_map_tskstinitcc_tskst = chlddts.col_id
                inner join tbl_taskcc chldtsk on chldmtsi.col_map_taskstateinitcctaskcc = chldtsk.col_id and chldtsk.col_casecctaskcc = v_CaseId
                inner join tbl_tw_workitemcc chldtwi on chldtsk.col_tw_workitemcctaskcc = chldtwi.col_id
                ...
                and (case when td.col_processorcode is not null and twi.col_activity = dts.col_activity then f_DCM_invokeTaskProcessor2(td.col_processorcode, td.col_id)
    ...
    for rec in (
       ...
       mtsi.col_id as TaskStateInitId, mtsi.col_processorcode as TaskStateInitProc, mtsi.col_assignprocessorcode as TaskStateInitAssignProc
                ...
                and lower(dim.col_code) = 'automatic_rule'
                and dts.col_activity in (select null from dual union select NextActivity from table(f_DCM_getNextActivityListCC(TaskId => tsk.col_id)))
                and exists (select col_taskdpchldcctaskstinitcc from tbl_taskdependencycc where col_taskdpchldcctaskstinitcc = mtsi.col_id and col_type = 'FSCO')
                and (tsk.col_id, mtsi.col_id) not in
                (select chldtsk.col_id as ChildsTaskId, chldmtsi.col_id as ChildMtsiId
                from (select col_id, col_taskccstp_resolutioncode, col_tw_workitemcctaskcc, col_taskccdict_tasksystype, col_casecctaskcc from tbl_taskcc where col_casecctaskcc = v_CaseId) tsk
                left  join tbl_stp_resolutioncode src on tsk.col_taskccstp_resolutioncode = src.col_id
                inner join tbl_tw_workitemcc twi on tsk.col_tw_workitemcctaskcc = twi.col_id
                inner join tbl_dict_tasksystype dtst on tsk.col_taskccdict_tasksystype = dtst.col_id
                ...
                inner join tbl_dict_taskstate dts on mtsi.col_map_tskstinitcc_tskst = dts.col_id
                inner join tbl_dict_initmethod dim on mtsi.col_map_tskstinitcc_initmtd = dim.col_id
                inner join tbl_taskdependencycc td on mtsi.col_id = td.col_taskdpprntcctaskstinitcc and td.col_type in ('FSCO')
                inner join tbl_map_taskstateinitcc chldmtsi on td.col_taskdpchldcctaskstinitcc = chldmtsi.col_id
                inner join tbl_dict_taskstate chlddts on chldmtsi.col_map_tskstinitcc_tskst = chlddts.col_id
                inner join tbl_taskcc chldtsk on chldmtsi.col_map_taskstateinitcctaskcc = chldtsk.col_id and chldtsk.col_casecctaskcc = v_CaseId
                inner join tbl_tw_workitemcc chldtwi on chldtsk.col_tw_workitemcctaskcc = chldtwi.col_id
    ...
    for rec in (select tsk.col_id as TaskId, tsk.col_taskid as TaskBsId, tsk.col_name as Taskname, dtst.col_code as TaskSysType, tsk.col_type as TaskType,
       tsk.col_dateassigned as TaskDateAssigned, tsk.col_dateclosed as TaskDateClosed,
       tsk.col_depth as TaskDepth,
       tsk.col_description as TaskDescription, twi.col_id as twId, twi.col_activity as twActivity, dts.col_activity as twTargetActivity,
       mtsi.col_id as TaskStateInitId, mtsi.col_processorcode as TaskStateInitProc, mtsi.col_assignprocessorcode as TaskStateInitAssignProc
                ...
                and dts.col_activity in (select null from dual union select NextActivity from table(f_DCM_getNextActivityListCC(TaskId => tsk.col_id)))
                and (tsk.col_id, mtsi.col_id) in
                (select s2.ChildTaskId as ChildTaskId, s2.ChildMtsiId as ChildMtsiId
                from
                (select s1.ChildTaskId, s1.ChildMtsiId, row_number() over (order by s1.RowNumber) as RowNumber
                from
                (select chldtsk.col_id as ChildTaskId, chldmtsi.col_id as ChildMtsiId, row_number() over (order by td.col_taskdependencyorder) as RowNumber
                from (select col_id, col_taskccstp_resolutioncode, col_tw_workitemcctaskcc, col_taskccdict_tasksystype, col_casecctaskcc from tbl_taskcc where col_casecctaskcc = v_CaseId) tsk
                left  join tbl_stp_resolutioncode src on tsk.col_taskccstp_resolutioncode = src.col_id
                inner join tbl_tw_workitemcc twi on tsk.col_tw_workitemcctaskcc = twi.col_id
                inner join tbl_dict_tasksystype dtst on tsk.col_taskccdict_tasksystype = dtst.col_id
                ...
                inner join tbl_dict_taskstate dts on mtsi.col_map_tskstinitcc_tskst = dts.col_id
                inner join tbl_dict_initmethod dim on mtsi.col_map_tskstinitcc_initmtd = dim.col_id
                inner join tbl_taskdependencycc td on mtsi.col_id = td.col_taskdpprntcctaskstinitcc and td.col_type in ('FSCEX')
                inner join tbl_map_taskstateinitcc chldmtsi on td.col_taskdpchldcctaskstinitcc = chldmtsi.col_id
                inner join tbl_dict_taskstate chlddts on chldmtsi.col_map_tskstinitcc_tskst = chlddts.col_id
                inner join tbl_taskcc chldtsk on chldmtsi.col_map_taskstateinitcctaskcc = chldtsk.col_id and chldtsk.col_casecctaskcc = v_CaseId
                inner join tbl_tw_workitemcc chldtwi on chldtsk.col_tw_workitemcctaskcc = chldtwi.col_id
                ...
                and chlddts.col_activity in (select null from dual union select NextActivity from table(f_DCM_getNextActivityListCC(TaskId => chldtsk.col_id)))
                and lower(chlddim.col_code) = 'automatic_rule'
                and ((twi.col_activity = dts.col_activity and chldmtsi.col_routedby is null)
                and (case when td.col_processorcode is not null and twi.col_activity = dts.col_activity then f_DCM_invokeTaskProcessor2(td.col_processorcode, td.col_id)
                              when chldmtsi.col_processorcode is not null and twi.col_activity = dts.col_activity then f_DCM_invokeTaskProcessor(chldmtsi.col_processorcode, chldmtsi.col_id)
                              else 1 end) = 1)
                union all
                select chldtsk.col_id as ChildTaskId, chldmtsi.col_id as ChildMtsiId, 999999 as RowNumber
                from (select col_id, col_taskccstp_resolutioncode, col_tw_workitemcctaskcc, col_taskccdict_tasksystype, col_casecctaskcc from tbl_taskcc where col_casecctaskcc = v_CaseId) tsk
                left  join tbl_stp_resolutioncode src on tsk.col_taskccstp_resolutioncode = src.col_id
                inner join tbl_tw_workitemcc twi on tsk.col_tw_workitemcctaskcc = twi.col_id
                inner join tbl_dict_tasksystype dtst on tsk.col_taskccdict_tasksystype = dtst.col_id
    ...
    loop
      for rec2 in (select chldtsk2.col_id as ChildTaskId, chldmtsi2.col_id as ChildMtsiId
                   from tbl_taskcc tsk
                   inner join tbl_map_taskstateinitcc mtsi on tsk.col_id = mtsi.col_map_taskstateinitcctaskcc
                   inner join tbl_taskdependencycc td on mtsi.col_id = td.col_taskdpprntcctaskstinitcc and td.col_type in ('FSCEX')
                   inner join tbl_map_taskstateinitcc chldmtsi on td.col_taskdpchldcctaskstinitcc = chldmtsi.col_id
                   inner join tbl_taskcc chldtsk on chldmtsi.col_map_taskstateinitcctaskcc = chldtsk.col_id
                   inner join tbl_taskdependencycc td2 on mtsi.col_id = td2.col_taskdpprntcctaskstinitcc and td.col_type in ('FSCEX')
                   inner join tbl_map_taskstateinitcc chldmtsi2 on td2.col_taskdpchldcctaskstinitcc = chldmtsi2.col_id
                   inner join tbl_taskcc chldtsk2 on chldmtsi2.col_map_taskstateinitcctaskcc = chldtsk2.col_id
                   where chldtsk.col_id = rec.TaskId)
      loop
        update tbl_map_taskstateinitcc set col_routedby = SYS_CONTEXT('CLIENTCONTEXT','AccessSubject'), col_routeddate = sysdate where col_id = rec2.ChildMtsiId;
      end loop;
      v_result := f_DCM_routeCaseCCTaskCCFn(CaseId => v_CaseId, TaskId => rec.TaskId, TaskStateInitAssignProc => rec.TaskStateInitAssignProc, TaskStateInitId => rec.TaskStateInitId, TwTargetActivity => rec.twTargetActivity);
      if v_result = -1 then
        return -1;
    ...
    for rec in (select tsk.col_id as TaskId, tsk.col_taskid as TaskBsId, tsk.col_name as Taskname, dtst.col_code as TaskSysType, tsk.col_type as TaskType,
       ...
       mtsi.col_id as TaskStateInitId, mtsi.col_processorcode as TaskStateInitProc, mtsi.col_assignprocessorcode as TaskStateInitAssignProc
                ...
                inner join tbl_dict_taskstate dts on mtsi.col_map_tskstinitcc_tskst = dts.col_id
                inner join tbl_dict_initmethod dim on mtsi.col_map_tskstinitcc_initmtd = dim.col_id
                inner join tbl_taskdependencycc td on mtsi.col_id = td.col_taskdpprntcctaskstinitcc and td.col_type in ('FSCIN')
                inner join tbl_map_taskstateinitcc chldmtsi on td.col_taskdpchldcctaskstinitcc = chldmtsi.col_id
                inner join tbl_dict_taskstate chlddts on chldmtsi.col_map_tskstinitcc_tskst = chlddts.col_id
                inner join tbl_taskcc chldtsk on chldmtsi.col_map_taskstateinitcctaskcc = chldtsk.col_id and chldtsk.col_casecctaskcc = v_CaseId
                inner join tbl_tw_workitemcc chldtwi on chldtsk.col_tw_workitemcctaskcc = chldtwi.col_id
    ...
    for rec in (select tsk.col_id as TaskId, tsk.col_taskid as TaskBsId, tsk.col_name as Taskname, dtst.col_code as TaskSysType, tsk.col_type as TaskType,
       ...
       mtsi.col_id as TaskStateInitId, mtsi.col_processorcode as TaskStateInitProc, mtsi.col_assignprocessorcode as TaskStateInitAssignProc
                ...
                inner join tbl_dict_taskstate dts on mtsi.col_map_tskstinitcc_tskst = dts.col_id
                inner join tbl_dict_initmethod dim on mtsi.col_map_tskstinitcc_initmtd = dim.col_id
                inner join tbl_taskdependencycc td on mtsi.col_id = td.col_taskdpprntcctaskstinitcc and td.col_type in ('FSCP')
                inner join tbl_map_taskstateinitcc chldmtsi on td.col_taskdpchldcctaskstinitcc = chldmtsi.col_id
                inner join tbl_dict_taskstate chlddts on chldmtsi.col_map_tskstinitcc_tskst = chlddts.col_id
                inner join tbl_taskcc chldtsk on chldmtsi.col_map_taskstateinitcctaskcc = chldtsk.col_id and chldtsk.col_casecctaskcc = v_CaseId
                inner join tbl_tw_workitemcc chldtwi on chldtsk.col_tw_workitemcctaskcc = chldtwi.col_id
    ...
    if v_CaseInvalid = 1 then
      v_CaseInvalid := 0;
      v_result := f_DBG_createDBGTrace(CaseId => v_CaseId, Location => 'Before recurrent call to DCM_routeCaseCCTasksCCFn3',
                                       Message => 'Case ' || to_char(v_CaseId) || ' before reccurent call to DCM_routeCaseCCTasksCCFn3', Rule => 'DCM_routeCaseCCTasksCCFn3', TaskId => null);
      v_result := f_DCM_routeCaseCCTasksCCFn3(CaseId => v_CaseId);
      v_result := f_DBG_createDBGTrace(CaseId => v_CaseId, Location => 'After recurrent call to DCM_routeCaseCCTasksCCFn3',
                                       Message => 'Case ' || to_char(v_CaseId) || ' after reccurent call to DCM_routeCaseCCTasksCCFn3', Rule => 'DCM_routeCaseCCTasksCCFn3', TaskId => null);
    end if;
    v_result := f_DBG_createDBGTrace(CaseId => v_CaseId, Location => 'DCM_routeCaseCCTasksCCFn3 end',
                                     Message => 'DCM_routeCaseCCTasksCCFn3 end for case ' || to_char(v_CaseId), Rule => 'DCM_routeCaseCCTasksCCFn3', TaskId => null);
end;