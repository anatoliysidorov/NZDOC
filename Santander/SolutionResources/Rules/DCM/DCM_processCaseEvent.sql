...
begin
  ...
  for rec in (select cs.col_id as col_id, ce.col_processorcode as col_processorcode,
                ...
                inner join tbl_dict_casestate css on csi.col_map_csstinit_csst = css.col_id
                --JOIN TO TASK/CASE EVENT TYPE DICTIONARY (EXAMPLE: "VALIDATION")
                inner join tbl_dict_taskeventtype tet on ce.col_taskeventtypecaseevent = tet.col_id
                --JOIN TO TASK/CASE EVENT MOMENT DICTIONARY (EXAMPLE: "BEFORE_ASSIGN")
                inner join tbl_dict_taskeventmoment tem on ce.col_taskeventmomentcaseevent = tem.col_id
