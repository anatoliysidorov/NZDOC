...
begin
  begin
    select Allowed into v_result from table(f_dcm_getCaseworkerAccessFn2(p_AccessObjectId => :AccessObjectId, p_CaseId => null,
                                                       ...
                                                       p_PermissionId => (select col_id from tbl_ac_permission where col_code = 'EXECUTE' and col_permissionaccessobjtype =
                                                         (select col_id from tbl_ac_accessobjecttype where col_code = 'CASE_TRANSITION')),
