...
begin
  ...
  loop
    ...
    if rec.IsFunction = 1 then
      ...
      --VV: was before cache for rec2 in (select col_paramcode as ParamCode, col_paramvalue as ParamValue from tbl_autoruleparameter where col_taskeventautoruleparam = (select col_id from tbl_taskevent where col_id = rec.TaskEventId))
