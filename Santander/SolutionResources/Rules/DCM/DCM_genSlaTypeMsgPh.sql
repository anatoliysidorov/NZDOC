...
BEGIN
  ...
  IF v_inCache = 1 THEN
	...
	LEFT JOIN tbl_DICT_SlaEventType et ON et.col_id = se.COL_SLAEVENTCC_SLAEVENTTYPE
    WHERE se.col_id = v_slaeventid;
  ELSE
    SELECT et.col_name
    INTO :PlaceholderResult
    FROM tbl_SlaEvent se
	LEFT JOIN tbl_DICT_SlaEventType et ON et.col_id = se.COL_SLAEVENTDICT_SLAEVENTTYPE
    WHERE se.col_id = v_slaeventid;
  END IF;
