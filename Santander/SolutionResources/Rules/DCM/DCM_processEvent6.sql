...
begin
  ...
  v_result := f_DBG_createDBGTrace(CaseId => v_CaseId, Location => 'DCM_processEvent6 before validation begins',
                                   Message => 'Task ' || to_char(v_NextTaskId) || ' before validation event processing', Rule => 'DCM_processEvent6', TaskId => v_NextTaskId);
  ...
  loop
    ...
    if rec.IsFunction = 1 then
      v_result := f_DBG_createDBGTrace(CaseId => v_CaseId, Location => 'DCM_processEvent6 before invocation of validation processor',
                                       ...
                                       Rule => 'DCM_processEvent6', TaskId => v_NextTaskId);
      ...
      v_result := f_DBG_createDBGTrace(CaseId => v_CaseId, Location => 'DCM_processEvent6 after invocation of validation processor',
