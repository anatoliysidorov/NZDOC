...
FROM (select tv.*
           ...
           ((select tsk.col_id
              ...
              <%=IfNotNull(":Task_Id", " and (tsk.col_id = :Task_Id)")%>
               start with (case when :Parent_Id is not null then tsk.col_id else tsk.col_parentid end) = nvl(:Parent_Id,0)
               connect by prior tsk.col_id = tsk.col_parentid
               ) s1
               inner join vw_dcm_simpletask tv on s1.col_id = tv.id)) tv
...
WHERE 1=1
    ...
    <%=IfNotNull(":TASKSTATEIDS", " AND tv.TaskState_id IN (SELECT TO_NUMBER(COLUMN_VALUE) FROM TABLE(asf_splitclob(:TASKSTATEIDS, ',')))")%>
