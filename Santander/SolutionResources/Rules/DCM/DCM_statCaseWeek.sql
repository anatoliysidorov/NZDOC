...
begin
  ...
  if v_count > 0 then
    begin
      ...
      select count(*) into v_count2
        ...
        and dee.col_datevalue between trunc(sysdate, 'D') and trunc(sysdate + interval '7' day, 'D');
      exception
        when NO_DATA_FOUND then
          v_count2 := 0;
    end;
  ...
  v_CaseComplete := 1;
  begin
    select cst.col_id, cwu.Id, to_char(trunc(dee.col_datevalue, 'D')), (dee.col_datevalue - des.col_datevalue) * 24 * 60
  ...
  if (v_CaseComplete is not null) and (v_CaseSysType is not null) and (v_CaseWorkerId is not null) and (v_YearMonth is not null) then
    begin
      select col_id into v_id
        ...
        where col_statcaseweekcasesystype = v_CaseSysType
        and col_statcaseweekcaseworker = v_Caseworkerid
    ...
    if v_id is not null then
      begin
        select col_avginprocess, col_totalinprocess, col_totalclosed into v_AvgInProcess, v_TotalInProcess, v_TotalClosed from tbl_statcaseweek where col_id = v_id;
        exception
          when NO_DATA_FOUND then
            v_AvgInProcess := 0;
            v_TotalInProcess := 0;
      ...
      insert into tbl_statcaseweek(col_statcaseweekcaseworker, col_statcaseweekcasesystype, col_yearmonth, col_totalclosed, col_meetgoal, col_nomeetgoal, col_meetdeadline, col_nomeetdeadline, col_avginprocess, col_totalinprocess)
